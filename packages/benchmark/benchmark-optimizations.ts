...
import { benchmarkWithGC, compareBenchmarks, type BenchmarkResult } from './benchmark-gc-pressure.js';
import { welchTTest, formatStatisticalReport, type TTestResult } from './statistical-analysis.js';
...
interface ParserVariant {
  name: string;
  description: string;
  Parser: typeof BaselineParser;
}

interface WriterVariant {
  name: string;
  description: string;
  Writer: typeof BaselineWriter;
}

const parserVariants: ParserVariant[] = [
  {
    name: 'baseline',
...
const writerVariants: WriterVariant[] = [
  {
    name: 'baseline',
...
// Load test patterns
function getTestPatterns(): TestPattern[] {
...
async function runComprehensiveBenchmark(options: {
  ...
  if (patterns.length === 0) {
    ...
    console.error('Run: pnpm tsx generate-test-xmls.ts');
    process.exit(1);
  }
    ...
    for (const variant of parserVariants) {
      ...
      try {
        ...
        const result = await benchmarkWithGC(
          `${variant.name}-parser-${pattern.name}`,
          () => {
            parser.reset();
            parser.write(xml);
            parser.end();
        ...
        if (variant.name === 'baseline') {
          ...
          if (baseline) {
            ...
            console.log(formatStatisticalReport(
              baseline.times,
              result.times,
              'Baseline',
              variant.name
  ...
  for (const pattern of patterns) {
    ...
    for (const variant of writerVariants) {
      ...
      try {
        // Run benchmark
        const result = await benchmarkWithGC(
          `${variant.name}-writer-${pattern.name}`,
          () => {
...
// Print summary table
function printSummaryTable(results: ComprehensiveResult[]): void {
...
function generateRecommendations(results: ComprehensiveResult[]): void {
  const issues: string[] = [];
  const successes: string[] = [];

  for (const result of results) {
    if (!result.comparison || !result.statisticalTest) continue;
    ...
    if (significant && improvement < -5) {
      issues.push(
        `⚠️  ${variant} on ${pattern}: ${Math.abs(improvement).toFixed(1)}% slower (p=${result.statisticalTest.pValue.toFixed(4)})`
      );
    }
    ...
    if (memoryImprovement < -50) {
      issues.push(
        `⚠️  ${variant} on ${pattern}: ${Math.abs(memoryImprovement).toFixed(1)}% more memory usage`
      );
    }
    ...
    if (gcReduction < -50) {
      issues.push(
        `⚠️  ${variant} on ${pattern}: ${Math.abs(gcReduction).toFixed(1)}% more GC events`
      );
    }
    ...
    if (significant && improvement > 5 && memoryImprovement > -10) {
      successes.push(
        `✓ ${variant} on ${pattern}: ${improvement.toFixed(1)}% faster, ${memoryImprovement.toFixed(1)}% memory improvement`
      );
    }
  }
...
export { runComprehensiveBenchmark, getTestPatterns, type ComprehensiveResult };
