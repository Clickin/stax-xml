...
function testScaling(elementCount: number): ScalingTestResult {
  // Baseline
...
for (const size of sizes) {
  ...
  console.log(
    `${String(size).padStart(8)} | ${result.baseline.toFixed(2).padStart(8)}ms | ` +
...
function calculateGrowthRate(results: ScalingTestResult[], field: keyof ScalingTestResult): string {
  ...
  if (!small || !large) return 'N/A';

  const smallTime = small[field] as number;
  const largeTime = large[field] as number;

  const ratio = largeTime / smallTime;

  // O(n) would be ~10x, O(n log n) would be ~13x, O(n²) would be ~100x
  if (ratio < 12) return `~O(n) [${ratio.toFixed(1)}x]`;
  if (ratio < 20) return `~O(n log n) [${ratio.toFixed(1)}x]`;
  if (ratio < 50) return `~O(n log n) or worse [${ratio.toFixed(1)}x]`;
