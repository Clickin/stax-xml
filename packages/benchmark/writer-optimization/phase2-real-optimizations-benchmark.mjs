 ...
 */
function runBenchmark(testName, elementCount, generateXml) {
 ...
 */
function generateAttributeHeavy(WriterClass, count) {
 ...
 */
function generateTextHeavy(WriterClass, count) {
    const writer = new WriterClass();
    writer.writeStartDocument();
    writer.writeStartElement('root');
    for (let i = 0; i < count; i++) {
 ...
 */
function generateTextNoEntities(WriterClass, count) {
    const writer = new WriterClass();
    writer.writeStartDocument();
    writer.writeStartElement('root');
    for (let i = 0; i < count; i++) {
 ...
 */
function generateMixedContent(WriterClass, count) {
    const writer = new WriterClass();
    writer.writeStartDocument();
    writer.writeStartElement('root');
    for (let i = 0; i < count; i++) {
 ...
 */
function generateDeepNested(WriterClass, count) {
    const writer = new WriterClass();
    writer.writeStartDocument();
    writer.writeStartElement('root');
    const depth = 5;
    const itemsPerLevel = Math.ceil(Math.pow(count, 1 / depth));
    function writeLevel(currentDepth, remaining) {
        if (currentDepth >= depth || remaining <= 0)
            return remaining;
 ...
 */
function generateRealistic(WriterClass, count) {
    const writer = new WriterClass();
    writer.writeStartDocument();
...
function generateReport(summary) {
    ...
    // Footer
    markdown += `---\n\n`;
    markdown += `*Generated by phase2-real-optimizations-benchmark.mjs*\n`;
