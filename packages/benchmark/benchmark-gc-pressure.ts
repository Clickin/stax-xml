...
export class GCMonitor {
  ...
  private heapBefore: v8.HeapStatistics;
  private heapAfter: v8.HeapStatistics | null = null;
...
export async function benchmarkWithGC(
  name: string,
  ...
  for (let i = 0; i < warmupIterations; i++) {
    await fn();
    if (i % 5 === 0) forceGC();  // GC every 5 warmup iterations
  }
...
export function compareBenchmarks(baseline: BenchmarkResult, optimized: BenchmarkResult): {
  ...
  const memoryImprovement =
    (baseline.gcMetrics.heapUsedDelta - optimized.gcMetrics.heapUsedDelta) /
