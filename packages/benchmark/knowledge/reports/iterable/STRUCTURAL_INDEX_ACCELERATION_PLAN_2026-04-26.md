...
The next bounded lowering path accepts a descriptor for root array/object plans whose item selector is a single descendant element and whose fields are scalar relative attributes or scalar one-segment relative child elements. Native now prefers the direct descriptor parser exposed as `parseObjectRowsUint8Array`, with `parseObjectRowsViaTableUint8Array` retained as the fallback and benchmark comparator. It returns columnar string and typed number fields; TypeScript reconstructs objects while applying compiled scalar validation so number range/integer semantics and unsupported plan fallback remain owned by the converter. Unsupported plans and missing backend capabilities fall back to the existing table or JavaScript paths.

Platform package verification must stage the freshly built native addon into the target package and smoke the projection exports through that package entrypoint. Checking only `parseAggregateBuffer` is insufficient because converter acceleration depends on `parseItemRowsViaTableUint8Array`, `parseObjectRowsUint8Array`, and `parseObjectRowsViaTableUint8Array`.

Benchmark-only staged platform breakdowns showed that direct generic parsing removes the table-build-then-replay bottleneck, but the generic public converter path still misses the 1.5x gate. Stable-shape JavaScript hydration is faster in isolation but too small to recover the gap alone. Removing direct-parser attribute span collection avoids temporary allocation but does not move the public benchmark materially, so the larger remaining cost is native generic string materialization, N-API transfer of per-field string columns, and JavaScript row hydration. Do not add production object-shape codegen before measuring fewer eagerly materialized string columns or another coarse schema-specific native projection tier.

The release comparator matrix should include simdxml as a benchmark comparator, but only behind an explicit external runner and a size cap. The current simdxml crate parses into a flat structural index rather than a DOM tree, but the benchmark adapter still requires whole-file bytes plus structural arrays, so `node-string-return` and `cross-runtime-comparison` cap simdxml fixture size at 64 MiB by default via `--simdxml-max-mib`.
