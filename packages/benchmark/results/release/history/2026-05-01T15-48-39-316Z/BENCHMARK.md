...
Generated: 2026-05-01T15:48:39.316Z
Run ID: 2026-05-01T15-48-39-316Z
...
- `Converter`: public declarative converter schema returning the same object shape through `schema.parseSync()`.
...
|---------|--------------|----------------|--------------|-------|
| **EventReaderSync** | 229.74 µs | ~4,353 ops/sec | 45.42 kb | Public string event iterator building the same object shape manually |
| **StreamReaderSync** | 217.33 µs | ~4,601 ops/sec | 26.10 kb | Public byte-stream pull reader building the same object shape manually |
| **ProjectionReader** | 4.22 µs | ~237,144 ops/sec | 0.41 kb | Public projection surface returning the same object shape through object records |
| **Converter** | 265.04 µs | ~3,773 ops/sec | 172.71 kb | Declarative schema building the same object shape through the converter facade |
...
| Library | Average Time | Operations/sec | Memory Usage | Notes |
|---------|--------------|----------------|--------------|-------|
| **EventReaderSync** | 317.86 µs | ~3,146 ops/sec | 84.44 kb | Public string event iterator building the same object shape manually |
| **StreamReaderSync** | 309.98 µs | ~3,226 ops/sec | 40.41 kb | Public byte-stream pull reader building the same object shape manually |
| **ProjectionReader** | 15.42 µs | ~64,838 ops/sec | 2.82 kb | Public projection surface returning the same object shape through object records |
| **Converter** | 551.69 µs | ~1,813 ops/sec | 560.15 kb | Declarative schema building the same object shape through the converter facade |
...
| Library | Average Time | Operations/sec | Memory Usage | Notes |
|---------|--------------|----------------|--------------|-------|
| **EventReaderSync** | 119.78 ms | ~8.35 ops/sec | 34.46 mb | Public string event iterator building the same object shape manually |
| **StreamReaderSync** | 89.27 ms | ~11.2 ops/sec | 16.95 mb | Public byte-stream pull reader building the same object shape manually |
| **ProjectionReader** | 22.83 ms | ~43.81 ops/sec | 1.66 mb | Public projection surface returning the same object shape through object records |
| **Converter** | 552.00 ms | ~1.81 ops/sec | 478.84 mb | Declarative schema building the same object shape through the converter facade |
...
| Library | Average Time | Operations/sec | Memory Usage | Notes |
|---------|--------------|----------------|--------------|-------|
| **EventReaderSync** | 880.20 ms | ~1.14 ops/sec | 101.70 mb | Public string event iterator building the same object shape manually |
| **StreamReaderSync** | 630.81 ms | ~1.59 ops/sec | 62.44 mb | Public byte-stream pull reader building the same object shape manually |
| **ProjectionReader** | 172.89 ms | ~5.78 ops/sec | 12.46 mb | Public projection surface returning the same object shape through object records |
| **Converter** | 4.26 s | ~0.23 ops/sec | 3.12 gb | Declarative schema building the same object shape through the converter facade |
...
|-----------|-------------|-----------------|--------------|------------|
| 1MiB temp file | **EventReader stream native** | 64.65 ms | 26.33 mb | 15.47 MiB/s |
| 10MiB temp file | **EventReader stream native** | 447.32 ms | 95.44 mb | 22.36 MiB/s |
| 100MiB temp file | **EventReader stream native** | 4.08 s | 234.74 mb | 24.49 MiB/s |
| 1GiB temp file | **EventReader stream native** | 40.87 s | 234.77 mb | 25.05 MiB/s |
