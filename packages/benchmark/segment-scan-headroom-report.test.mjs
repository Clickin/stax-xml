...
const tmpDir = join(__dirname, 'results', 'tmp', 'segment-scan-headroom-report-test');
const jsonOut = join(tmpDir, 'segment-scan-headroom.json');
const mdOut = join(tmpDir, 'segment-scan-headroom.md');

test('segment scan headroom report compares concat and no-concat partial byte scans', () => {
  resetTmp();
  const result = spawnSync(process.execPath, [
    '--expose-gc',
    join(__dirname, 'segment-scan-headroom.mjs'),
    '--file',
  ...
  assert.equal(report.objective, 'segment-scan-headroom');
  assert.equal(report.contract, 'file-backed-delimiter-scan-headroom');
  ...
  for (const row of report.rows) {
    ...
    assert.equal(row.contractScope, 'delimiter-byte-scan-no-xml-parse-no-string-materialization');
    ...
    assert.equal(row.delimiterCount, first.delimiterCount);
  ...
  const concat = report.rows.find(row => row.id === 'grouped-concat-scan');
  const segmented = report.rows.find(row => row.id === 'grouped-segment-scan');
  assert.equal(concat.concatBeforeScan, true);
  assert.equal(concat.segmentAwareScan, false);
  assert.equal(segmented.concatBeforeScan, false);
  assert.equal(segmented.segmentAwareScan, true);

  const markdown = readFileSync(mdOut, 'utf8');
  assert.match(markdown, /# Segment Scan Headroom/);
  assert.match(markdown, /Grouped segment \/ concat ratio/);
  assert.match(markdown, /same-byte-scan-contract/);
  assert.match(markdown, /segment-scan-headroom/);
  assert.match(markdown, /partial-not-stax-counterexample/);
  assert.match(markdown, /source-contract/);
  assert.match(markdown, /delimiter byte-scan probe only/);
});
