...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 720\s+measured rows/);
  assert.match(markdown, /425 JavaScript 1 GiB\+ full-string rows and zero\s+bounded-memory 200 MiB\/s\+ counterexamples/);
  ...
  assert.match(markdown, /recognizes 720\s+measured rows/);
  assert.match(markdown, /89 benchmark artifacts, 16 source artifacts, 8\s+trace\/profile artifacts, 13 allocation artifacts, 2 environment artifacts, and\s+9 negative-result artifacts/);
