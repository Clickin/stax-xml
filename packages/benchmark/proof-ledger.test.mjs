...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 931\s+sample throughput rows and 99 aggregate rows/);
  assert.match(markdown, /588 JavaScript 1 GiB\+\s+full-string sample rows plus 77 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /258 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  ...
  assert.match(markdown, /all 258\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /recognizes 931\s+measured rows/);
  assert.match(markdown, /131 benchmark artifacts, 17 source artifacts, 10\s+trace\/profile artifacts, 15 allocation artifacts, 2 environment artifacts, and\s+16 negative-result artifacts/);
