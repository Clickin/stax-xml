...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /191 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has 141 JavaScript 1 GiB\+ full-string rows, 132 of\s+them bounded/);
  ...
  assert.match(markdown, /all 191\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /repeated text-value cache at 0\.74x of\s+its control, bounded long-text cache at 0\.83x of its control, long ASCII text\s+fast path at 0\.41x of its control, medium ASCII text fast path at 1\.04x of its\s+control, fold-trim checksum at 0\.84x of its control, the byte-boundary trim\s+guard at 1\.00x of its control, ASCII byte pre-trim before decode at 0\.99x of its\s+control, and manual ASCII materialization for all string spans at 0\.63x of its\s+control/);
