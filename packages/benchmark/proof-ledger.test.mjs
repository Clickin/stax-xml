...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 830\s+sample throughput rows and 93 aggregate rows/);
  ...
  assert.match(markdown, /`sync-iterable-byte-batches` records parser input\s+`synchronous Iterable<Uint8Array\[\]>`, `directReadableStream=false`, and\s+116\.62 MiB\/s/);
  assert.match(markdown, /backpressure-respecting `web-readable-stream-pull` row\s+records parser input `Web ReadableStream<Uint8Array>`,\s+`directReadableStream=true`, and 131\.19 MiB\/s/);
  ...
  assert.match(markdown, /30 threshold-crossing partial\/projection rows/);
  ...
  assert.match(markdown, /Bun\/JSC `scanAllNoDecode` at 326\.65 MiB\/s from\s+`candidate-headroom-cross-process-books-corpus-partial\.json` remains the\s+fastest parser-produced partial row/);
  ...
  assert.match(markdown, /recognizes 830\s+measured rows/);
  assert.match(markdown, /116 benchmark artifacts, 16 source artifacts, 10\s+trace\/profile artifacts, 15 allocation artifacts, 2 environment artifacts, and\s+12 negative-result artifacts/);
  ...
  assert.match(markdown, /reports `125\.58 MiB\/s` for the sync iterable path with 16\.2% sample\s+spread and `132\.99 MiB\/s` for the backpressure-respecting ReadableStream path\s+with 3\.1% sample spread/);
