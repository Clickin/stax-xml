...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 526\s+measured rows/);
  assert.match(markdown, /287 JavaScript 1 GiB\+ full-string rows and zero\s+bounded-memory 200 MiB\/s\+ counterexamples/);
  assert.match(markdown, /Bun\/JSC `rawFrameNameId` from\s+`candidate-headroom-cross-process-books-corpus\.json` at 183\.10 MiB\/s/);
  ...
  assert.match(markdown, /Node\/V8 `rawFrameNameId`\s+averaged `173\.26 MiB\/s` with `2\.0%` spread/);
  assert.match(markdown, /Bun\/JSC\s+`rawFrameNameId` averaged `181\.51 MiB\/s` with `1\.8%` spread/);
