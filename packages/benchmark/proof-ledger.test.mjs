...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 743\s+sample throughput rows and 89 aggregate rows/);
  ...
  assert.match(markdown, /recognizes 743\s+measured rows/);
  ...
  assert.match(markdown, /Derived `summary\.fastest` \/ `summary\.slowest` projections are excluded from row\s+extraction/);
  assert.match(markdown, /source-consumption rows are not counted twice/);
