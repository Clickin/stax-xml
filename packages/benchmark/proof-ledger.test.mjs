...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /fastest partial row was\s+`scanAllNoDecode` at `153\.75 MiB\/s`/);
  assert.match(markdown, /fastest full-string parity row was\s+`rawFrameNameId` at `82\.08 MiB\/s`/);
  ...
  assert.match(markdown, /max used JS heap `14\.4 MiB`/);
  ...
  assert.match(markdown, /fastest partial row was `scanAllNoDecode` at\s+`172\.61 MiB\/s`/);
  assert.match(markdown, /fastest full-string parity\s+row was `rawFrameDirect` at `51\.64 MiB\/s`/);
  ...
  assert.match(markdown, /max used JS heap `349\.7 MiB` for `stringFull`/);
  assert.match(markdown, /not a proof of\s+ArrayBuffer external-memory accounting/);
