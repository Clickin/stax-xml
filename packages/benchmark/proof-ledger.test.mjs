...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /269 aggregated rows and 223 JavaScript 1 GiB\+\s+full-string rows/);
  assert.match(markdown, /zero 200 MiB\/s\+ bounded-memory JavaScript\s+counterexamples/);
  assert.match(markdown, /current scan covers 215 primary release JSON artifacts/);
  ...
  assert.match(markdown, /all 217 JavaScript 1 GiB\+ full-string\s+rows with source-mode metadata in this aggregate are marked as not full\s+`ArrayBuffer` parser-input rows/);
  assert.match(markdown, /separately records 134 corpus-seed replay\s+rows, with a maximum seed size of 100\.26 MiB and a maximum seed\/target ratio of\s+0\.09/);
  assert.match(markdown, /classifies memory on the same 223\s+JavaScript 1 GiB\+ full-string rows used by the counterexample scan: 206 rows\s+are bounded, 17 are unbounded or unproven/);
  ...
  assert.match(markdown, /recognizes 1,209\s+sample throughput rows and 170 aggregate rows/);
  ...
  assert.match(markdown, /814 JavaScript 1 GiB\+\s+full-string sample rows plus 133 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /440 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has\s+348 JavaScript 1 GiB\+ full-string rows, 339 of\s+them bounded/);
  ...
  assert.match(markdown, /recognizes 1,209\s+measured rows/);
  assert.match(markdown, /current audit scans 215 primary release artifacts/);
  assert.match(markdown, /153 benchmark artifacts, 24 source artifacts, 15\s+trace\/profile artifacts, 16 allocation artifacts, 4 environment artifacts, and\s+24 negative-result artifacts, 814 JavaScript 1 GiB\+ full-string rows/);
  ...
  assert.match(markdown, /same-contract memory frontier from that\s+aggregate: 223 JavaScript 1 GiB\+ full-string memory rows, 206 bounded rows, and\s+17 unbounded or unproven rows/);
