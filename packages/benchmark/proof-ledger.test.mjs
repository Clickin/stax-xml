...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /229 aggregated rows and 196 JavaScript 1 GiB\+\s+full-string rows/);
  ...
  assert.match(markdown, /all 190 JavaScript 1 GiB\+ full-string\s+rows with source-mode metadata in this aggregate are marked as not full\s+`ArrayBuffer` parser-input rows/);
  assert.match(markdown, /separately records 113 corpus-seed replay\s+rows, with a maximum seed size of 100\.26 MiB and a maximum seed\/target ratio of\s+0\.09/);
  assert.match(markdown, /classifies memory on the same 196\s+JavaScript 1 GiB\+ full-string rows used by the counterexample scan: 179 rows\s+are bounded, 17 are unbounded or unproven/);
  ...
  assert.match(markdown, /recognizes 1,081\s+sample throughput rows and 136 aggregate rows/);
  ...
  assert.match(markdown, /729 JavaScript 1 GiB\+\s+full-string sample rows plus 112 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /370 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has\s+278 JavaScript 1 GiB\+ full-string rows, 269 of\s+them bounded/);
  ...
  assert.match(markdown, /all 370\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /recognizes 1081\s+measured rows/);
  assert.match(markdown, /145 benchmark artifacts, 18 source artifacts, 10\s+trace\/profile artifacts, 15 allocation artifacts, 4 environment artifacts, and\s+19 negative-result artifacts, 729 JavaScript 1 GiB\+ full-string rows/);
  ...
  assert.match(markdown, /all\s+190 JavaScript 1 GiB\+ full-string rows with source-mode metadata are not full\s+`ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /same-contract memory frontier from that\s+aggregate: 196 JavaScript 1 GiB\+ full-string memory rows, 179 bounded rows, and\s+17 unbounded or unproven rows/);
