...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 474\s+measured rows/);
  assert.match(markdown, /250 JavaScript 1 GiB\+ full-string rows and zero\s+bounded-memory 200 MiB\/s\+ counterexamples/);
  ...
  assert.match(markdown, /recognizes 474\s+measured rows/);
  assert.match(markdown, /55 benchmark artifacts, 9 source artifacts, 3\s+trace\/profile artifacts, 9 allocation artifacts/);
  assert.match(markdown, /two release corpus seeds: `books\.xml` and `treebank_e\.xml`/);
  assert.match(markdown, /92 Chrome\/V8 browser rows, 69 Firefox\/SpiderMonkey\s+browser rows, zero Safari\/WebKit browser rows, and 69 non-V8 browser benchmark\s+rows/);
