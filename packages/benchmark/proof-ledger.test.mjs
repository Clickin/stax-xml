...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /215 aggregated rows and 184 JavaScript 1 GiB\+\s+full-string rows/);
  ...
  assert.match(markdown, /all 178 JavaScript 1 GiB\+ full-string\s+rows with source-mode metadata in this aggregate are marked as not full\s+`ArrayBuffer` parser-input rows/);
  assert.match(markdown, /separately records 101 corpus-seed replay\s+rows, with a maximum seed size of 100\.26 MiB and a maximum seed\/target ratio of\s+0\.09/);
  assert.match(markdown, /classifies memory on the same 184\s+JavaScript 1 GiB\+ full-string rows used by the counterexample scan: 167 rows\s+are bounded, 17 are unbounded or unproven/);
  ...
  assert.match(markdown, /recognizes 999\s+sample throughput rows and 117 aggregate rows/);
  ...
  assert.match(markdown, /653 JavaScript 1 GiB\+\s+full-string sample rows plus 95 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /334 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  ...
  assert.match(markdown, /all 334\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /38 threshold-crossing partial\/projection rows/);
  ...
  assert.match(markdown, /recognizes 999\s+measured rows/);
  assert.match(markdown, /135 benchmark artifacts, 18 source artifacts, 10\s+trace\/profile artifacts, 15 allocation artifacts, 4 environment artifacts, and\s+19 negative-result artifacts, 653 JavaScript 1 GiB\+ full-string rows/);
  ...
  assert.match(markdown, /all\s+178 JavaScript 1 GiB\+ full-string rows with source-mode metadata are not full\s+`ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /same-contract memory frontier from that\s+aggregate: 184 JavaScript 1 GiB\+ full-string memory rows, 167 bounded rows, and\s+17 unbounded or unproven rows/);
