...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /fastest partial row was\s+`scanAllNoDecode` at `149\.91 MiB\/s`/);
  assert.match(markdown, /fastest full-string parity row was\s+`rawFrameNameId` at `75\.37 MiB\/s`/);
  ...
  assert.match(markdown, /max used JS heap `16\.0 MiB`/);
  assert.match(markdown, /max working set `440\.6 MiB`/);
  assert.match(markdown, /max private bytes `192\.6 MiB`/);
  ...
  assert.match(markdown, /fastest partial row was `scanAllNoDecode` at\s+`94\.60 MiB\/s`/);
  assert.match(markdown, /fastest full-string parity\s+row was `stringFull` at `29\.15 MiB\/s`/);
  assert.match(markdown, /All full rows preserved 75,206,126 events and checksum\s+`-925527041`/);
  ...
  assert.match(markdown, /max used JS heap `349\.6 MiB` for `stringFull`/);
  assert.match(markdown, /max working set `833\.1 MiB`/);
  assert.match(markdown, /max private bytes\s+`653\.3 MiB`/);
