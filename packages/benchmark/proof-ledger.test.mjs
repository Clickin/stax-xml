...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /234 aggregated rows and 201 JavaScript 1 GiB\+\s+full-string rows/);
  ...
  assert.match(markdown, /all 195 JavaScript 1 GiB\+ full-string\s+rows with source-mode metadata in this aggregate are marked as not full\s+`ArrayBuffer` parser-input rows/);
  assert.match(markdown, /separately records 118 corpus-seed replay\s+rows, with a maximum seed size of 100\.26 MiB and a maximum seed\/target ratio of\s+0\.09/);
  assert.match(markdown, /classifies memory on the same 201\s+JavaScript 1 GiB\+ full-string rows used by the counterexample scan: 184 rows\s+are bounded, 17 are unbounded or unproven/);
  ...
  assert.match(markdown, /recognizes 1,102\s+sample throughput rows and 143 aggregate rows/);
  ...
  assert.match(markdown, /747 JavaScript 1 GiB\+\s+full-string sample rows plus 118 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /388 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has\s+296 JavaScript 1 GiB\+ full-string rows, 287 of\s+them bounded/);
  ...
  assert.match(markdown, /all 388\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /recognizes 1,102\s+measured rows/);
  assert.match(markdown, /147 benchmark artifacts, 18 source artifacts, 10\s+trace\/profile artifacts, 15 allocation artifacts, 4 environment artifacts, and\s+19 negative-result artifacts, 747 JavaScript 1 GiB\+ full-string rows/);
  ...
  assert.match(markdown, /all\s+195 JavaScript 1 GiB\+ full-string rows with source-mode metadata are not full\s+`ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /same-contract memory frontier from that\s+aggregate: 201 JavaScript 1 GiB\+ full-string memory rows, 184 bounded rows, and\s+17 unbounded or unproven rows/);
