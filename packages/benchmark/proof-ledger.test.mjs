...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /190 aggregated rows and 166 JavaScript 1 GiB\+\s+full-string rows/);
  ...
  assert.match(markdown, /all 160 JavaScript 1 GiB\+ full-string\s+rows with source-mode metadata in this aggregate are marked as not full\s+`ArrayBuffer` parser-input rows/);
  assert.match(markdown, /separately records 85 corpus-seed replay\s+rows, with a maximum seed size of 100\.26 MiB and a maximum seed\/target ratio of\s+0\.09/);
  ...
  assert.match(markdown, /recognizes 930\s+sample throughput rows and 99 aggregate rows/);
  assert.match(markdown, /587 JavaScript 1 GiB\+\s+full-string sample rows plus 77 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /257 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has 180 JavaScript 1 GiB\+ full-string rows, 171 of\s+them bounded/);
  assert.match(markdown, /file-backed sync byte-batch bucket adds 49 full-string rows,\s+48 of them bounded, with fastest row 152\.11 MiB\/s/);
  ...
  assert.match(markdown, /`sync-iterable-byte-batches` now covers three grouped sync rows/);
  assert.match(markdown, /`sync-iterable-byte-batches`, which records parser input\s+`synchronous Iterable<Uint8Array\[\]>`, `directReadableStream=false`, and\s+75\.36 MiB\/s/);
  ...
  assert.match(markdown, /all 257\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /recognizes 930\s+measured rows/);
  assert.match(markdown, /130 benchmark artifacts, 17 source artifacts, 10\s+trace\/profile artifacts, 15 allocation artifacts, 2 environment artifacts, and\s+16 negative-result artifacts/);
