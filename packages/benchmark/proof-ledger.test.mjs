...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /277 aggregated rows and 230 JavaScript 1 GiB\+\s+full-string rows/);
  assert.match(markdown, /zero 200 MiB\/s\+ bounded-memory JavaScript\s+counterexamples/);
  assert.match(markdown, /current scan covers 216 primary release JSON artifacts/);
  ...
  assert.match(markdown, /all 224 JavaScript 1 GiB\+ full-string\s+rows with source-mode metadata in this aggregate are marked as not full\s+`ArrayBuffer` parser-input rows/);
  assert.match(markdown, /separately records 141 corpus-seed replay\s+rows, with a maximum seed size of 100\.26 MiB and a maximum seed\/target ratio of\s+0\.09/);
  assert.match(markdown, /classifies memory on the same 230\s+JavaScript 1 GiB\+ full-string rows used by the counterexample scan: 213 rows\s+are bounded, 17 are unbounded or unproven/);
  ...
  assert.match(markdown, /recognizes 1,217\s+sample throughput rows and 170 aggregate rows/);
  ...
  assert.match(markdown, /821 JavaScript 1 GiB\+\s+full-string sample rows plus 133 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /447 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has\s+355 JavaScript 1 GiB\+ full-string rows, 346 of\s+them bounded/);
  ...
  assert.match(markdown, /all 433\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /recognizes 1,217\s+measured rows/);
  assert.match(markdown, /current audit scans 216 primary release artifacts/);
  assert.match(markdown, /154 benchmark artifacts, 24 source artifacts, 15\s+trace\/profile artifacts, 16 allocation artifacts, 4 environment artifacts, and\s+24 negative-result artifacts, 821 JavaScript 1 GiB\+ full-string rows/);
  ...
  assert.match(markdown, /same-contract memory frontier from that\s+aggregate: 230 JavaScript 1 GiB\+ full-string memory rows, 213 bounded rows, and\s+17 unbounded or unproven rows/);
