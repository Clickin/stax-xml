...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /327 JavaScript 1 GiB\+ full-string rows and zero\s+bounded-memory 200 MiB\/s\+ counterexamples/);
  ...
  assert.match(markdown, /repeat the same public `EventReader` source-boundary comparison on the 1\.00 GiB\s+`treebank_e\.xml` corpus-cycle fixture/);
  assert.match(markdown, /corpus source is split into 64 KiB\s+`Uint8Array` chunks and repeats only complete corpus cycles/);
  assert.match(markdown, /All corpus rows\s+preserved 75,206,126 events and checksum `1421140645`/);
  assert.match(markdown, /On Node\/V8,\s+`readableStreamBatch1` was fastest at `66\.52 MiB\/s`/);
  assert.match(markdown, /On Bun\/JSC, `readableStreamBatch1` reported `56\.64 MiB\/s`/);
  assert.match(markdown, /On Deno\/V8,\s+`readableStreamBatch1` reported `56\.70 MiB\/s`/);
  assert.match(markdown, /scenario-dependent result: it helped\s+the generated tiny-row fixture, but it did not reveal corpus-cycle headroom/);
