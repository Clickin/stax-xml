...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 537\s+measured rows/);
  assert.match(markdown, /292 JavaScript 1 GiB\+ full-string rows and zero\s+bounded-memory 200 MiB\/s\+ counterexamples/);
  ...
  assert.match(markdown, /recognizes 537\s+measured rows/);
  assert.match(markdown, /62 benchmark artifacts, 11 source artifacts, 5\s+trace\/profile artifacts, 12 allocation artifacts,\s+and 1 environment artifact/);
  ...
  assert.match(markdown, /ReadableStream rows reported 29\.55 MiB\/s at batch size 1,\s+35\.35 MiB\/s at/);
  assert.match(markdown, /async byte-batch rows\s+reported 39\.92 MiB\/s at batch size 16 and 40\.85 MiB\/s at batch size 64/);
  assert.match(markdown, /validates async-boundary headroom and preserves backpressure/);
