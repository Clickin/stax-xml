...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /`sync-iterable-byte-batches-batch-8`, which records parser input\s+`synchronous Iterable<Uint8Array\[\]>`, `directReadableStream=false`, and\s+91\.95 MiB\/s/);
  assert.match(markdown, /focused audit now includes seven source-shape rows/);
  ...
  assert.match(markdown, /`async-iterable-raw-frame-ascii-batch-8` at 72\.94 MiB\/s/);
  ...
  assert.match(markdown, /fastest backpressure-respecting `web-readable-stream-pull`\s+row is `web-readable-stream-raw-frame-ascii-batch-8`; it records parser input\s+`Web ReadableStream<Uint8Array>`, `directReadableStream=true`,\s+`respectsBackpressure=true`, and 75\.20 MiB\/s in the focused audit/);
