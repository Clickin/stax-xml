...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 727\s+measured rows/);
  assert.match(markdown, /432 JavaScript 1 GiB\+ full-string rows and zero\s+bounded-memory 200 MiB\/s\+ counterexamples/);
  assert.match(markdown, /Bun\/JSC `rawFrameNameId` from\s+`bun-candidate-headroom-books-corpus-stability\.json` at 178\.52 MiB\/s/);
  ...
  assert.match(markdown, /recognizes 727\s+measured rows/);
  assert.match(markdown, /92 benchmark artifacts, 16 source artifacts, 8\s+trace\/profile artifacts, 13 allocation artifacts, 2 environment artifacts, and\s+9 negative-result artifacts/);
