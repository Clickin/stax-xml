...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 524\s+measured rows/);
  ...
  assert.match(markdown, /Bun\/JSC `scanAllNoDecode` at 333\.40 MiB\/s/);
  ...
  assert.match(markdown, /Bun\/JSC `scanAllNoDecode` reached `333\.40 MiB\/s`/);
  assert.match(markdown, /Node\/V8\s+`scanAllNoDecode` reached `273\.80 MiB\/s`/);
  assert.match(markdown, /Chrome\/V8 browser\s+`scanAllNoDecode` reached `206\.76 MiB\/s`/);
  assert.match(markdown, /Bun\/JSC\s+`rawFrameNameId` was the fastest single-run full row at `171\.51 MiB\/s`/);
  assert.match(markdown, /Node\/V8 `rawFrameNameId` reported `170\.87 MiB\/s`/);
