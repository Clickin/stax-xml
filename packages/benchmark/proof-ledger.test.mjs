...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 557\s+measured rows/);
  assert.match(markdown, /312 JavaScript 1 GiB\+ full-string rows and zero\s+bounded-memory 200 MiB\/s\+ counterexamples/);
  ...
  assert.match(markdown, /recognizes 557\s+measured rows/);
  assert.match(markdown, /66 benchmark artifacts, 11 source artifacts, 5\s+trace\/profile artifacts, 12 allocation artifacts,\s+and 1 environment artifact/);
