...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /116 aggregated rows and 105 JavaScript 1 GiB\+\s+full-string rows/);
  ...
  assert.match(markdown, /same-fixture 1024 MiB JS row vs Woodstox target now includes the\s+file-backed source and batch-size sweep rows: `stax-raw-frame-name-id-chunk-32kib`\s+is fastest at 151\.70 MiB\/s, 0\.80x Woodstox, and 19\.95 MiB\/s below the 0\.9x\s+target/);
  ...
  assert.match(markdown, /recognizes 767\s+sample throughput rows and 89 aggregate rows/);
  assert.match(markdown, /476 JavaScript 1 GiB\+\s+full-string sample rows plus 69 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /142 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  ...
  assert.match(markdown, /recognizes 767\s+measured rows/);
  assert.match(markdown, /99 benchmark artifacts, 16 source artifacts, 10\s+trace\/profile artifacts, 15 allocation artifacts, 2 environment artifacts, and\s+10 negative-result artifacts/);
