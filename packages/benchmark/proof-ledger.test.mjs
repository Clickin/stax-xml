...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /150 JavaScript 1 GiB\+ full-string rows and zero\s+bounded-memory 200 MiB\/s\+ counterexamples/);
  ...
  assert.match(markdown, /recognizes 353\s+measured rows/);
  assert.match(markdown, /42 benchmark artifacts, 9 source artifacts, 3\s+trace\/profile artifacts, 9 allocation artifacts/);
  assert.match(markdown, /one release corpus seed: `treebank_e\.xml`/);
  assert.match(markdown, /73 Chrome\/V8 browser rows, 11 Firefox\/SpiderMonkey\s+browser rows, zero Safari\/WebKit browser rows, and 11 non-V8 browser benchmark\s+rows/);
  ...
  assert.match(markdown, /Firefox\s+TextDecoder and projection stability\/fresh-process rows/);
