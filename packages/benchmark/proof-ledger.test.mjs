...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 511\s+measured rows/);
  ...
  assert.match(markdown, /recognizes 511\s+measured rows/);
  assert.match(markdown, /60 benchmark artifacts, 11 source artifacts, 4\s+trace\/profile artifacts, 11 allocation artifacts/);
  ...
  assert.match(markdown, /3 open or partial obligations/);
  ...
  assert.match(markdown, /Bun\/JSC `scanAllNoDecode` reached `334\.63 MiB\/s`/);
  assert.match(markdown, /Node\/V8\s+`scanAllNoDecode` reached `279\.08 MiB\/s`/);
  ...
  assert.match(markdown, /fresh runtime processes repeated the same selected rows/);
  assert.match(markdown, /Node\/V8 `rawFrameNameId`\s+averaged `166\.68 MiB\/s` with `3\.6%` spread/);
  assert.match(markdown, /Bun\/JSC\s+`rawFrameNameId` averaged `171\.04 MiB\/s` with `3\.1%` spread/);
