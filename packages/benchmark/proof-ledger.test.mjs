...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 874\s+sample throughput rows and 93 aggregate rows/);
  assert.match(markdown, /545 JavaScript 1 GiB\+\s+full-string sample rows plus 73 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /215 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has 163 JavaScript 1 GiB\+ full-string rows, 154 of\s+them bounded/);
  assert.match(markdown, /file-backed sync byte-batch bucket adds 47 full-string rows,\s+46 of them bounded, with fastest row 152\.11 MiB\/s/);
  ...
  assert.match(markdown, /`sync-iterable-byte-batches-batch-8`, which records parser input\s+`synchronous Iterable<Uint8Array\[\]>`, `directReadableStream=false`, and\s+128\.29 MiB\/s/);
  assert.match(markdown, /backpressure-respecting `web-readable-stream-pull` row\s+records parser input `Web ReadableStream<Uint8Array>`,\s+`directReadableStream=true`, and 125\.22 MiB\/s/);
  ...
  assert.match(markdown, /all 215\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /recognizes 874\s+measured rows/);
  ...
  assert.match(markdown, /64 KiB file-backed basis with sync batch sizes 1, 8, and 16 plus\s+the backpressure-respecting ReadableStream row, with three measured runs/);
  assert.match(markdown, /reports `118\.57 MiB\/s` for batch size 1, `128\.29 MiB\/s` for batch size 8,\s+`124\.65 MiB\/s` for batch size 16, and `125\.22 MiB\/s` for the direct\s+ReadableStream path/);
  assert.match(markdown, /fastest sync grouped row is therefore only 1\.02x the\s+ReadableStream row in this run/);
  ...
  assert.match(markdown, /All rows are\s+parser-demand-driven, while only the Web ReadableStream row carries stream\s+backpressure metadata/);
