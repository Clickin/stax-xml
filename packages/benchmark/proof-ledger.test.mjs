...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /221 aggregated rows and 188 JavaScript 1 GiB\+\s+full-string rows/);
  ...
  assert.match(markdown, /all 182 JavaScript 1 GiB\+ full-string\s+rows with source-mode metadata in this aggregate are marked as not full\s+`ArrayBuffer` parser-input rows/);
  assert.match(markdown, /separately records 105 corpus-seed replay\s+rows, with a maximum seed size of 100\.26 MiB and a maximum seed\/target ratio of\s+0\.09/);
  assert.match(markdown, /classifies memory on the same 188\s+JavaScript 1 GiB\+ full-string rows used by the counterexample scan: 171 rows\s+are bounded, 17 are unbounded or unproven/);
  ...
  assert.match(markdown, /recognizes 1,057\s+sample throughput rows and 128 aggregate rows/);
  ...
  assert.match(markdown, /705 JavaScript 1 GiB\+\s+full-string sample rows plus 104 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /346 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has\s+254 JavaScript 1 GiB\+ full-string rows, 245 of\s+them bounded/);
  ...
  assert.match(markdown, /all 346\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /recognizes 1057\s+measured rows/);
  assert.match(markdown, /144 benchmark artifacts, 18 source artifacts, 10\s+trace\/profile artifacts, 15 allocation artifacts, 4 environment artifacts, and\s+19 negative-result artifacts, 705 JavaScript 1 GiB\+ full-string rows/);
  ...
  assert.match(markdown, /all\s+182 JavaScript 1 GiB\+ full-string rows with source-mode metadata are not full\s+`ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /same-contract memory frontier from that\s+aggregate: 188 JavaScript 1 GiB\+ full-string memory rows, 171 bounded rows, and\s+17 unbounded or unproven rows/);
