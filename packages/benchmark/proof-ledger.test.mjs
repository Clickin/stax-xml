...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 348\s+measured rows/);
  assert.match(markdown, /147 JavaScript 1 GiB\+ full-string rows and zero\s+bounded-memory 200 MiB\/s\+ counterexamples/);
  ...
  assert.match(markdown, /recognizes 348\s+measured rows/);
  assert.match(markdown, /41 benchmark artifacts, 9 source artifacts, 3\s+trace\/profile artifacts, 9 allocation artifacts/);
  assert.match(markdown, /one release corpus seed: `treebank_e\.xml`/);
  assert.match(markdown, /73 Chrome\/V8 browser rows, 6 Firefox\/SpiderMonkey browser\s+rows, zero Safari\/WebKit browser rows, and 6 non-V8 browser benchmark rows/);
