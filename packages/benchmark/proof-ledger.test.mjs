...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /232 aggregated rows and 199 JavaScript 1 GiB\+\s+full-string rows/);
  ...
  assert.match(markdown, /all 193 JavaScript 1 GiB\+ full-string\s+rows with source-mode metadata in this aggregate are marked as not full\s+`ArrayBuffer` parser-input rows/);
  assert.match(markdown, /separately records 116 corpus-seed replay\s+rows, with a maximum seed size of 100\.26 MiB and a maximum seed\/target ratio of\s+0\.09/);
  assert.match(markdown, /classifies memory on the same 199\s+JavaScript 1 GiB\+ full-string rows used by the counterexample scan: 182 rows\s+are bounded, 17 are unbounded or unproven/);
  ...
  assert.match(markdown, /recognizes 1,096\s+sample throughput rows and 141 aggregate rows/);
  ...
  assert.match(markdown, /741 JavaScript 1 GiB\+\s+full-string sample rows plus 116 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /382 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has\s+290 JavaScript 1 GiB\+ full-string rows, 281 of\s+them bounded/);
  ...
  assert.match(markdown, /all 382\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /recognizes 1096\s+measured rows/);
  assert.match(markdown, /146 benchmark artifacts, 18 source artifacts, 10\s+trace\/profile artifacts, 15 allocation artifacts, 4 environment artifacts, and\s+19 negative-result artifacts, 741 JavaScript 1 GiB\+ full-string rows/);
  ...
  assert.match(markdown, /all\s+193 JavaScript 1 GiB\+ full-string rows with source-mode metadata are not full\s+`ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /same-contract memory frontier from that\s+aggregate: 199 JavaScript 1 GiB\+ full-string memory rows, 182 bounded rows, and\s+17 unbounded or unproven rows/);
  ...
  assert.match(markdown, /access-shape-candidate-cross-process\.md/);
  assert.match(markdown, /Node\/V8\s+reported `cursorAccessor` at `161\.48 MiB\/s` with `4\.7%` spread/);
  assert.match(markdown, /`rawFrameDirect` at `136\.98 MiB\/s` with `33\.2%` spread, and `rawFrameNameId` at\s+`147\.13 MiB\/s`/);
  assert.match(markdown, /Bun\/JSC reported `cursorAccessor` at\s+`167\.04 MiB\/s`/);
  assert.match(markdown, /`rawFrameNameId` at `177\.34 MiB\/s` with `3\.2%` spread/);
  assert.match(markdown, /fastest individual Bun\/JSC `rawFrameNameId` sample at `179\.70 MiB\/s`/);
  assert.match(markdown, /fresh runtime processes with\s+`processRuns=3`, child `runs=1`, and `warmups=0`/);
