...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /237 aggregated rows and 204 JavaScript 1 GiB\+\s+full-string rows/);
  ...
  assert.match(markdown, /all 198 JavaScript 1 GiB\+ full-string\s+rows with source-mode metadata in this aggregate are marked as not full\s+`ArrayBuffer` parser-input rows/);
  assert.match(markdown, /separately records 121 corpus-seed replay\s+rows, with a maximum seed size of 100\.26 MiB and a maximum seed\/target ratio of\s+0\.09/);
  assert.match(markdown, /classifies memory on the same 204\s+JavaScript 1 GiB\+ full-string rows used by the counterexample scan: 187 rows\s+are bounded, 17 are unbounded or unproven/);
  ...
  assert.match(markdown, /recognizes 1,111\s+sample throughput rows and 146 aggregate rows/);
  ...
  assert.match(markdown, /756 JavaScript 1 GiB\+\s+full-string sample rows plus 121 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /397 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has\s+305 JavaScript 1 GiB\+ full-string rows, 296 of\s+them bounded/);
  ...
  assert.match(markdown, /all 397\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /all\s+198 JavaScript 1 GiB\+ full-string rows with source-mode metadata are not full\s+`ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /same-contract memory frontier from that\s+aggregate: 204 JavaScript 1 GiB\+ full-string memory rows, 187 bounded rows, and\s+17 unbounded or unproven rows/);
