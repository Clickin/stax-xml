...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /zero measured\s+rows with unknown full-string parity and 71 rows with unknown bounded-memory\s+flags/);
  assert.match(markdown, /unknown bounded-memory set contains 55 JavaScript rows, 56\s+full-string rows, 40 JavaScript full-string rows, 0 JavaScript 1 GiB\+\s+full-string rows, and 24 rows that have raw memory counters/);
  ...
  assert.match(markdown, /omitted 16,987,392 text\/CDATA string fields and changed the\s+checksum to `1372281363`/);
  ...
  assert.match(markdown, /recognizes 725\s+measured rows/);
  assert.match(markdown, /93 benchmark artifacts, 16 source artifacts, 8\s+trace\/profile artifacts, 13 allocation artifacts, 2 environment artifacts, and\s+9 negative-result artifacts/);
