...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 810\s+sample throughput rows and 89 aggregate rows/);
  assert.match(markdown, /504 JavaScript 1 GiB\+\s+full-string sample rows plus 69 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /174 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has 126 JavaScript 1 GiB\+ full-string rows, 117 of\s+them bounded/);
  ...
  assert.match(markdown, /all 174 JavaScript\s+1 GiB\+ full-string rows with source-mode metadata are now marked as not full\s+`ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /recognizes 810\s+measured rows/);
  assert.match(markdown, /112 benchmark artifacts, 16 source artifacts, 10\s+trace\/profile artifacts, 15 allocation artifacts, 2 environment artifacts, and\s+12 negative-result artifacts/);
