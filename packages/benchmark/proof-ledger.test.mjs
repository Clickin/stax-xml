...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /203 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has 153 JavaScript 1 GiB\+ full-string rows, 144 of\s+them bounded/);
  ...
  assert.match(markdown, /all 203\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
  ...
  assert.match(markdown, /36 threshold-crossing partial\/projection rows/);
