...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /all 210 JavaScript 1 GiB\+ full-string\s+rows with source-mode metadata in this aggregate are marked as not full\s+`ArrayBuffer` parser-input rows/);
  assert.match(markdown, /separately records 127 corpus-seed replay\s+rows, with a maximum seed size of 100\.26 MiB and a maximum seed\/target ratio of\s+0\.09/);
  assert.match(markdown, /classifies memory on the same 216\s+JavaScript 1 GiB\+ full-string rows used by the counterexample scan: 199 rows\s+are bounded, 17 are unbounded or unproven/);
  ...
  assert.match(markdown, /recognizes 1,201\s+sample throughput rows and 170 aggregate rows/);
  ...
  assert.match(markdown, /807 JavaScript 1 GiB\+\s+full-string sample rows plus 133 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /433 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has\s+341 JavaScript 1 GiB\+ full-string rows, 332 of\s+them bounded/);
  ...
  assert.match(markdown, /same-contract memory frontier from that\s+aggregate: 216 JavaScript 1 GiB\+ full-string memory rows, 199 bounded rows, and\s+17 unbounded or unproven rows/);
