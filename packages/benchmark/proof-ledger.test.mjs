...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /108 aggregated rows and 97 JavaScript 1 GiB\+\s+full-string rows/);
  ...
  assert.match(markdown, /same-fixture 1024 MiB JS row vs Woodstox target now includes the\s+file-backed source and batch-size sweep rows: `stax-raw-frame-name-id-batch-4`\s+is fastest at 146\.08 MiB\/s, 0\.77x Woodstox, and 25\.57 MiB\/s below the 0\.9x\s+target/);
  ...
  assert.match(markdown, /recognizes 759\s+sample throughput rows and 89 aggregate rows/);
  assert.match(markdown, /468 JavaScript 1 GiB\+\s+full-string sample rows plus 69 JavaScript 1 GiB\+ full-string aggregate rows/);
  ...
  assert.match(markdown, /134 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  ...
  assert.match(markdown, /recognizes 759\s+measured rows/);
