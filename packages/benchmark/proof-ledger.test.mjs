...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /recognizes 623\s+measured rows/);
  assert.match(markdown, /375 JavaScript 1 GiB\+ full-string rows and zero\s+bounded-memory 200 MiB\/s\+ counterexamples/);
  ...
  assert.match(markdown, /recognizes 623\s+measured rows/);
  assert.match(markdown, /72 benchmark artifacts, 12 source artifacts, 7\s+trace\/profile artifacts, 13 allocation artifacts, 1 environment artifact, and 2\s+negative-result artifacts/);
  assert.match(markdown, /three release corpus seeds:\s+`books\.xml`, `large\.xml`, and `treebank_e\.xml`/);
  assert.match(markdown, /98 Chrome\/V8 browser rows, 81 Firefox\/SpiderMonkey\s+browser rows, zero Safari\/WebKit browser rows, and 81 non-V8 browser benchmark\s+rows/);
