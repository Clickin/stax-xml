...
test('proof ledger keeps runtime-limit claims below conclusion strength', () => {
  ...
  assert.match(markdown, /209 JavaScript 1 GiB\+\s+full-string rows with source mode metadata/);
  assert.match(markdown, /generated-sync bucket now has 159 JavaScript 1 GiB\+ full-string rows, 150 of\s+them bounded/);
  ...
  assert.match(markdown, /all 209\s+JavaScript 1 GiB\+ full-string rows with source-mode metadata are now marked as\s+not full `ArrayBuffer` parser-input rows/);
