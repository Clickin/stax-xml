...
test('browser candidate headroom matrix supports a corpus-cycle fixture seed', (t) => {
  ...
  assert.equal(fetchReadable.sourceMode, 'fetch-readable-stream-pull');
  assert.equal(fetchReadable.demandDrivenSource, true);
  assert.equal(fetchReadable.directReadableStream, true);
  assert.equal(fetchReadable.respectsBackpressure, true);
  assert.equal(fetchReadable.fullArrayBufferParserInput, false);
  ...
  assert.equal(fetchAsyncBatch.sourceMode, 'fetch-async-iterable-byte-batches');
  assert.equal(fetchAsyncBatch.demandDrivenSource, true);
  assert.equal(fetchAsyncBatch.directReadableStream, false);
  assert.equal(fetchAsyncBatch.respectsBackpressure, true);
  assert.equal(fetchAsyncBatch.fullArrayBufferParserInput, false);
  ...
  assert.ok(report.findings.some(entry => entry.id === 'browser-fetch-readable-stream-source'));
