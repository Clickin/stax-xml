...
function generateQuickTestXML(size) {
    switch (size) {
        case 'small':
            return `<?xml version="1.0"?>
<root>
  ${Array.from({ length: 100 }, (_, i) => `<item id="${i}" name="item${i}" value="${Math.random()}">${Math.random().toString(36)}</item>`).join('\n  ')}
</root>`;
        case 'medium':
            return `<?xml version="1.0"?>
<root>
  ...
  <record id="${i}">
    <data>${Array.from({ length: 10 }, (_, j) => `<field name="field${j}">${Math.random().toString(36)}</field>`).join('')}</data>
  </record>`).join('')}
</root>`;
        case 'large':
            return `<?xml version="1.0"?>
<catalog>
  ...
  <product id="P${i}" sku="SKU${i}" category="cat${i % 10}">
    ...
    <attributes>
      ${Array.from({ length: 5 }, (_, j) => `<attr name="attr${j}" value="${Math.random().toString(36)}"/>`).join('\n      ')}
    </attributes>
  </product>`).join('')}
</catalog>`;
    }
...
async function quickBenchmark(options) {
    const { baseline: baselinePath, optimized: optimizedPath, testSize = 'medium', iterations = 50, showHeapInfo = false } = options;
    ...
    const baselineResult = await benchmarkWithGC('baseline', () => {
    ...
    const optimizedResult = await benchmarkWithGC('optimized', () => {
    ...
    const stats = formatStatisticalReport(baselineResult.times, optimizedResult.times, 'Baseline', 'Optimized');
    console.log(stats);
