...
test('stax public reader host API boundary audit pins current TextDecoder surface', () => {
  ...
  assert.equal(report.summary.primarySyncByteBatchRequiresTextDecoder, false);
  assert.equal(report.summary.asciiPrimarySyncByteBatchRequiresTextDecoder, false);
  assert.equal(report.summary.utf8FallbackDecoder, true);
  assert.equal(report.summary.nonUtf8RequiresTextDecoder, true);
  ...
  assert.deepEqual(report.summary.primarySyncByteBatchRequiredGlobals, ['Uint8Array']);
  assert.deepEqual(report.summary.asciiPrimarySyncByteBatchRequiredGlobals, ['Uint8Array']);
  assert.deepEqual(report.summary.directReadableStreamRequiredGlobals, ['Uint8Array', 'ReadableStream']);
  assert.deepEqual(report.summary.stringInputRequiredGlobals, ['TextEncoder']);
  assert.deepEqual(report.summary.rootImportRequiredGlobals, []);
  assert.deepEqual(report.summary.asyncWriterOutputRequiredGlobals, ['TextEncoder', 'WritableStream']);
  assert.deepEqual(report.summary.syncWriterOutputRequiredGlobals, []);
  ...
  assert.ok(report.checks.some(check => check.id === 'iterable-reader-utf8-fallback-without-textdecoder'));
  ...
  assert.ok(report.findings.some(finding =>
    finding.id === 'stax-writer-textencoder-boundary'
    && finding.classification === 'SOURCE_FACT'
  ));
  assert.ok(report.findings.some(finding =>
  ...
  assert.match(markdown, /Primary sync byte-batch requires TextDecoder: false/);
  ...
  assert.match(markdown, /iterable-reader-utf8-fallback-without-textdecoder/);
