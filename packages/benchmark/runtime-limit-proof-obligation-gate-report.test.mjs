...
test('runtime-limit proof-obligation gate permits only a conservative non-conclusion ledger', () => {
  ...
  assert.equal(report.frontierAuditSnapshot.targetDistance.fastestJsVisibleInCounterexampleScan, true);
  ...
  assert.ok(report.frontierAuditSnapshot.guards.some(item => item.id === 'target-distance-row-visible-in-counterexample-scan' && item.satisfied));
  ...
  assert.equal(report.coverageSnapshot.spiderMonkeyClosureAudit.closestBlockedCandidateCount, 5);
  assert.deepEqual(report.coverageSnapshot.spiderMonkeyClosureAudit.closestBlockedCandidateSourceArtifacts, [
  ...
  assert.ok(report.coverageSnapshot.guards.some(item => item.id === 'spidermonkey-closure-audit-comparison-current' && item.satisfied));
  assert.ok(report.coverageSnapshot.guards.some(item => item.id === 'spidermonkey-closure-frontier-current' && item.satisfied));
...
test('runtime-limit proof-obligation gate fails if coverage omits SpiderMonkey closure-audit surface counts', () => {
  ...
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const coverage = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-coverage-audit.json'), 'utf8'));
  delete coverage.coverage.spiderMonkeyDiagnostics.closureAuditCandidateCount;
  delete coverage.coverage.spiderMonkeyDiagnostics.closureAuditDiagnosticRowGap;
  delete coverage.coverage.spiderMonkeyDiagnostics.closureAuditQualifiedClosureCount;
  writeFileSync(badCoverageJsonOut, `${JSON.stringify(coverage, null, 2)}\n`);
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.coverageSnapshot.guards.some(item =>
    item.id === 'spidermonkey-closure-audit-surface-visible'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /diagnostic row count, closure-audit candidate count/.test(error)));
...
test('runtime-limit proof-obligation gate fails if coverage omits named SpiderMonkey closure gap artifacts', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const coverage = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-coverage-audit.json'), 'utf8'));
  delete coverage.coverage.spiderMonkeyDiagnostics.closureAuditCandidateSourcesOutsideDiagnostics;
  coverage.coverage.spiderMonkeyDiagnostics.diagnosticSourcesOutsideClosureAudit = ['unexpected-diagnostic.json'];
  writeFileSync(badCoverageJsonOut, `${JSON.stringify(coverage, null, 2)}\n`);
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.coverageSnapshot.guards.some(item =>
    item.id === 'spidermonkey-closure-audit-gap-artifacts-visible'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /closure candidates outside curated diagnostics rows/.test(error)));

  const markdown = readFileSync(badCoverageGateMdOut, 'utf8');
  assert.match(markdown, /Gate pass: no/);
  assert.match(markdown, /SpiderMonkey closure candidates outside coverage diagnostics: none/);
  assert.match(markdown, /SpiderMonkey coverage diagnostics outside closure candidates: `unexpected-diagnostic\.json`/);
  assert.match(markdown, /spidermonkey-closure-audit-gap-artifacts-visible/);
});

test('runtime-limit proof-obligation gate fails if coverage has stale SpiderMonkey closure comparison freshness', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const coverage = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-coverage-audit.json'), 'utf8'));
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.coverageSnapshot.guards.some(item =>
    item.id === 'spidermonkey-closure-audit-comparison-current'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /SpiderMonkey closure audit comparison freshness/.test(error)));
...
test('runtime-limit proof-obligation gate fails if coverage loses SpiderMonkey closest blocked frontier', () => {
  ...
  assert.ok(closureAudit);
  closureAudit.summary.minimumBlockedRequirementCount = 5;
  closureAudit.summary.closestBlockedCandidateCount = 1;
  closureAudit.summary.closestBlockedCandidateSourceArtifacts = [
    'spidermonkey-taskcluster-debug-jsshell-codegen-audit.json',
  ];
  writeFileSync(badCoverageJsonOut, `${JSON.stringify(coverage, null, 2)}\n`);
  ...
  assert.ok(report.gate.errors.some(error => /SpiderMonkey closure frontier/.test(error)));
...
test('runtime-limit proof-obligation gate fails if Safari handoff omits target-distance refresh', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const safari = handoff.handoffs.find(item => item.id === 'safari-webkit-browser-row-handoff');
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.handoffSnapshot.guards.some(item =>
    item.id === 'safari-target-distance-recomputed-after-rows'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /target-distance-audit regeneration/.test(error)));
...
test('runtime-limit proof-obligation gate fails if Safari handoff omits structured evidence intake', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const safari = handoff.handoffs.find(item => item.id === 'safari-webkit-browser-row-handoff');
  delete safari.evidenceIntakeContract;
  writeFileSync(badHandoffJsonOut, `${JSON.stringify(handoff, null, 2)}\n`);
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.handoffSnapshot.guards.some(item =>
    item.id === 'safari-structured-evidence-intake-contract'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /structured required artifacts/.test(error)));

  const markdown = readFileSync(badHandoffGateMdOut, 'utf8');
  assert.match(markdown, /Gate pass: no/);
  assert.match(markdown, /safari-structured-evidence-intake-contract/);
});
...
test('runtime-limit proof-obligation gate fails if Safari handoff omits local host and harness runability details', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const safari = handoff.handoffs.find(item => item.id === 'safari-webkit-browser-row-handoff');
  ...
  assert.ok(report.gate.errors.some(error => /host\/harness runability/.test(error)));
...
test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff omits structured evidence intake', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.handoffSnapshot.guards.some(item =>
    item.id === 'spidermonkey-structured-evidence-intake-contract'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /structured required artifacts/.test(error)));
  ...
  const markdown = readFileSync(badHandoffGateMdOut, 'utf8');
  assert.match(markdown, /Gate pass: no/);
  assert.match(markdown, /spidermonkey-structured-evidence-intake-contract/);
});
...
test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff omits Taskcluster source revision', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const spiderMonkey = handoff.handoffs.find(item => item.id === 'spidermonkey-codegen-handoff');
  ...
  assert.ok(report.gate.errors.some(error => /sourceRevision/.test(error)));
...
test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff omits closure audit identity status counts', () => {
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.handoffSnapshot.guards.some(item =>
    item.id === 'spidermonkey-closure-audit-identity-statuses'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /non-StAX diagnostic identity status counts/.test(error)));
...
test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff omits selected-row comparison match counts', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const spiderMonkey = handoff.handoffs.find(item => item.id === 'spidermonkey-codegen-handoff');
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.handoffSnapshot.guards.some(item =>
    item.id === 'spidermonkey-selected-row-comparison-match-counts'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /selected-row comparison match counts/.test(error)));

  const markdown = readFileSync(badHandoffGateMdOut, 'utf8');
  assert.match(markdown, /Gate pass: no/);
  assert.match(markdown, /spidermonkey-selected-row-comparison-match-counts/);
});

test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff omits diagnostic workload metadata counts', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const spiderMonkey = handoff.handoffs.find(item => item.id === 'spidermonkey-codegen-handoff');
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.handoffSnapshot.guards.some(item =>
    item.id === 'spidermonkey-diagnostic-workload-metadata-not-row-identity'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /diagnostic workload metadata/.test(error)));

  const markdown = readFileSync(badHandoffGateMdOut, 'utf8');
  assert.match(markdown, /Gate pass: no/);
  assert.match(markdown, /spidermonkey-diagnostic-workload-metadata-not-row-identity/);
});

test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff omits codegen comparison freshness', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const spiderMonkey = handoff.handoffs.find(item => item.id === 'spidermonkey-codegen-handoff');
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.handoffSnapshot.guards.some(item =>
    item.id === 'spidermonkey-codegen-comparison-freshness'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /same-contract comparison generatedAt and row count/.test(error)));

  const markdown = readFileSync(badHandoffGateMdOut, 'utf8');
  assert.match(markdown, /Gate pass: no/);
  assert.match(markdown, /spidermonkey-codegen-comparison-freshness/);
});

test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff uses stale codegen comparison freshness', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const spiderMonkey = handoff.handoffs.find(item => item.id === 'spidermonkey-codegen-handoff');
  ...
  spiderMonkey.localClosure.blockers = spiderMonkey.localClosure.blockers
    .map(item => item.replace(
      ...
      'against same-contract comparison generatedAt=2000-01-01T00:00:00.000Z, comparisonRowCount=1',
    ));
  writeFileSync(badHandoffJsonOut, `${JSON.stringify(handoff, null, 2)}\n`);
  ...
  assert.ok(report.gate.errors.some(error => /current same-contract comparison generatedAt and row count/.test(error)));
...
test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff omits closing metadata missing field counts', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const spiderMonkey = handoff.handoffs.find(item => item.id === 'spidermonkey-codegen-handoff');
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.handoffSnapshot.guards.some(item =>
    item.id === 'spidermonkey-closing-metadata-missing-fields'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /closing metadata subfields/.test(error)));
...
test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff omits disallowed evidence class counts', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const spiderMonkey = handoff.handoffs.find(item => item.id === 'spidermonkey-codegen-handoff');
  ...
  assert.equal(report.gate.pass, false);
  assert.ok(report.handoffSnapshot.guards.some(item =>
    item.id === 'spidermonkey-disallowed-evidence-class-counts'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /diagnostic scope-guard evidence classes/.test(error)));
...
test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff omits named closest blocked candidates', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const spiderMonkey = handoff.handoffs.find(item => item.id === 'spidermonkey-codegen-handoff');
  ...
  assert.ok(report.gate.errors.some(error => /named closest blocked candidates/.test(error)));
...
test('runtime-limit proof-obligation gate fails if SpiderMonkey handoff omits closure frontier evidence-class blocker', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const handoff = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'runtime-proof-gap-handoff.json'), 'utf8'));
  const spiderMonkey = handoff.handoffs.find(item => item.id === 'spidermonkey-codegen-handoff');
...
test('runtime-limit proof-obligation gate fails if counterexample scan contract or coverage shape is stale', () => {
  ...
  delete scan.summary.aggregateCounterexampleCount;
  ...
  assert.equal(report.counterexampleSnapshot.scanAggregateCounterexampleCount, null);
  assert.ok(report.counterexampleSnapshot.guards.some(item =>
  ...
  assert.ok(report.counterexampleSnapshot.guards.some(item =>
    item.id === 'counterexample-scan-aggregate-surface'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error => /counterexample-scan-current-coverage-shape/.test(error)));
  assert.ok(report.gate.errors.some(error => /counterexample-scan-aggregate-surface/.test(error)));
...
test('runtime-limit proof-obligation gate fails if target-distance baseline row is absent from counterexample scan surface', () => {
  resetTmp();
  writeFileSync(goodLedger, createLedgerFixture('`HYPOTHESIS`'));
  const targetDistance = JSON.parse(readFileSync(join(__dirname, 'results', 'release', 'target-distance-audit.json'), 'utf8'));
  targetDistance.summary.sameFixture1024MiBWoodstoxTarget.fastestJsCaseId = 'not-in-counterexample-scan';
  writeFileSync(badTargetDistanceJsonOut, `${JSON.stringify(targetDistance, null, 2)}\n`);
  ...
  const report = JSON.parse(readFileSync(badTargetDistanceGateJsonOut, 'utf8'));
  assert.equal(report.gate.pass, false);
  assert.equal(report.frontierAuditSnapshot.targetDistance.fastestJsCaseId, 'not-in-counterexample-scan');
  assert.equal(report.frontierAuditSnapshot.targetDistance.fastestJsVisibleInCounterexampleScan, false);
  assert.ok(report.frontierAuditSnapshot.guards.some(item =>
    item.id === 'target-distance-row-visible-in-counterexample-scan'
    && !item.satisfied
  ));
  assert.ok(report.gate.errors.some(error =>
    /Missing frontier audit guard target-distance-row-visible-in-counterexample-scan/.test(error)
  ));

  const markdown = readFileSync(badTargetDistanceGateMdOut, 'utf8');
  assert.match(markdown, /Gate pass: no/);
  assert.match(markdown, /Target JS row visible in counterexample scan: no/);
  assert.match(markdown, /target-distance-row-visible-in-counterexample-scan/);
});
