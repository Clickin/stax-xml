...
class XmlGenerator {
  ...
  private static randomText(minLength: number, maxLength: number): string {
  ...
  static generateSmallSimple(): string {
...
<root>
  ...
  static generateMediumNested(depth: number = 10, breadth: number = 5): string {
    let xml = '<?xml version="1.0" encoding="UTF-8"?>\n';

    function generateElement(level: number): string {
      if (level === 0) {
      ...
      for (let i = 0; i < breadth; i++) {
        const attrs = Array.from({ length: 3 }, (_, j) =>
          `attr${j}="${XmlGenerator.randomString(10)}"`
        ).join(' ');
  ...
  static generateAttributeHeavy(elements: number = 100): string {
    let xml = '<?xml version="1.0" encoding="UTF-8"?>\n<root>\n';

    for (let i = 0; i < elements; i++) {
      const attrs = Array.from({ length: 20 }, (_, j) =>
        `attribute${j}="${XmlGenerator.randomString(50)}"`
      ).join('\n         ');
  ...
  static generateTextHeavy(paragraphs: number = 500): string {
    let xml = '<?xml version="1.0" encoding="UTF-8"?>\n<document>\n';
  ...
  static generateCDATAHeavy(sections: number = 100): string {
    let xml = '<?xml version="1.0" encoding="UTF-8"?>\n<root>\n';
  ...
  static generateNamespaceHeavy(): string {
    return `<?xml version="1.0" encoding="UTF-8"?>
<root xmlns:app="http://example.com/app"
  ...
  static generateMixedContent(elements: number = 200): string {
    let xml = '<?xml version="1.0" encoding="UTF-8"?>\n';
    ...
    <doc:content>
      ...
      <doc:list>
        ${Array.from({ length: 5 }, (_, j) =>
          `<doc:item>${XmlGenerator.randomText(5, 20)}</doc:item>`
        ).join('\n        ')}
      </doc:list>
    </doc:content>
  </doc:section>\n`;
  ...
  static generateLargeRealistic(): string {
    ...
    <product:categories>
      ${Array.from({ length: Math.floor(Math.random() * 5) + 1 }, () =>
        `<product:category>${XmlGenerator.randomString(10)}</product:category>`
      ).join('\n      ')}
    </product:categories>
    <product:attributes>
      ${Array.from({ length: Math.floor(Math.random() * 10) + 5 }, (_, j) =>
        `<product:attribute name="attr${j}">${XmlGenerator.randomString(20)}</product:attribute>`
      ).join('\n      ')}
    </product:attributes>
  </product:item>\n`;
    }
