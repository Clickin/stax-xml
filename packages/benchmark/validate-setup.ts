...
const checks: Array<() => ValidationResult> = [
  ...
  () => {
    const scripts = [
      'generate-test-xmls.ts',
      'benchmark-inlining.ts',
      'profile-parsers.ts',
      'compare-results.ts',
      'benchmark.config.ts'
    ];
  ...
  () => {
    if (!global.gc) {
      return {
        ...
        details: 'Run with: node --expose-gc or tsx --expose-gc'
      };
    }
...
async function validateSetup() {
  ...
  if (failedCount > 0) {
    ...
    console.log('  - Run with GC: tsx --expose-gc <script>');
  ...
  console.log('\nNext steps:');
  console.log('  1. Run benchmarks: pnpm run bench:inlining');
  console.log('  2. View results: pnpm run compare:latest');
  console.log('  3. Profile parsers: pnpm run profile:all');
  console.log('  4. Generate HTML report: pnpm run compare:html');
