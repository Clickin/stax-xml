...
function generateMarkdownReport(results: BenchmarkResults): string {
  ...
  const byPattern = new Map<string, typeof results.results>();
    ...
    if (baseline) {
      ...
      if (optimized.length > 0) {
        ...
        for (const opt of optimized) {
          if (!opt.comparison || !opt.statisticalTest) continue;
  ...
  for (const result of results.results) {
    if (result.variant === 'baseline' || !result.statisticalTest || !result.comparison) continue;
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: tsx generate-report.ts <results-file.json> [output-file.md]');
    console.error('');
    console.error('Example:');
    console.error('  tsx generate-report.ts results/benchmark-2024-01-15.json report.md');
    process.exit(1);
  }
  ...
  const results: BenchmarkResults = JSON.parse(content);
