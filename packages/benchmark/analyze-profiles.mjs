...
function analyzeProfile(profilePath) {
    ...
    sorted.forEach(([funcName, data], index) => {
        ...
        console.log(`${(index + 1).toString().padEnd(6)}${data.hits.toString().padEnd(10)}${percentage.padEnd(8)}${funcName}`);
    });
    console.log('-'.repeat(80));
    // Find parser-specific functions
    console.log('\n Parser-Specific Functions:');
    console.log('-'.repeat(80));
    const parserFunctions = sorted.filter(([funcName]) => funcName.includes('parse') ||
...
function compareProfiles(baselinePath, inlinedPath) {
    console.log('\n\n');
    console.log('='.repeat(80));
    console.log('COMPARISON: Baseline vs Inlined');
    console.log('='.repeat(80));
    const baselineProfile = JSON.parse(fs.readFileSync(baselinePath, 'utf8'));
    const inlinedProfile = JSON.parse(fs.readFileSync(inlinedPath, 'utf8'));
