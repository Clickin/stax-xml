...
async function profileWithV8Prof(config, xml, parser) {
  ...
  `;
    ...
    if (fs.existsSync(logFile)) {
        ...
        const processResult = childProcess.spawnSync(process.execPath, ['--prof-process', logFile], { cwd: profileDir, encoding: 'utf8', shell: true });
...
async function profileWithInspector(config, xml, parser) {
    ...
    session.disconnect();
    // Save profile
    const profilePath = path.join(profileDir, `${config.variant}-${config.testFile}-${Date.now()}.cpuprofile`);
    ...
    try {
        ...
        const speedscopeResult = childProcess.spawnSync('npx', ['speedscope', profilePath, '-o', profilePath.replace('.cpuprofile', '.speedscope.json')], { stdio: 'inherit', shell: true });
...
async function profileWithClinic(config, xml, parser) {
    ...
    try {
        ...
        if (result.status === 0) {
            ...
            if (profileDirs.length > 0) {
                ...
                childProcess.spawnSync('npx', ['clinic', 'flame', '--visualize-only', latestProfile], { stdio: 'inherit', shell: true });
                fs.unlinkSync(scriptPath);
                return {
                    variant: config.variant,
                    testFile: config.testFile,
...
async function main() {
    ...
    const variant = args[0] || 'inlined';
    const testFile = args[1] || 'medium-nested.xml';
    const method = args[2] || 'inspector';
    ...
    if (!fs.existsSync(testDataPath)) {
        ...
        console.error('Please run generate-test-xmls.mjs first');
        process.exit(1);
    }
    ...
    try {
        ...
        // Save metadata
        const metadataPath = path.join(__dirname, 'profiles', `profile-${variant}-${config.testFile}-metadata.json`);
        const metadata = {
...
async function profileAll() {
    ...
    const variants = ['baseline', 'inlined'];
    const allResults = [];
