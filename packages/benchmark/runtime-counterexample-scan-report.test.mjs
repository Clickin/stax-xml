...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.sourceModeRowCount, 422);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 315);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 223
    && entry.boundedFullStringRowCount === 214
    ...
    && entry.demandDrivenRows === 223
    && entry.directReadableStreamRows === 0
    && entry.notFullArrayBufferRows === 223
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Scanned artifacts: 188/);
  assert.match(markdown, /Measured rows recognized: 1020/);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string rows recognized: 674/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 422/);
  assert.match(markdown, /Partial\/projection threshold rows: 35/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 315/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 223 \| 223 \| 214 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 223 \| 0 \| 0 \| 223 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:223\/223, file-backed-sync-iterable-byte-batches:53\/53, async-iterable-byte-batches:15\/15/);
