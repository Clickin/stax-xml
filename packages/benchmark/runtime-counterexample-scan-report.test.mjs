...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.sourceModeRowCount, 284);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 206);
  assert.equal(report.summary.partialHeadroomRowCount, 37);
  assert.equal(report.summary.textMaterializationHeadroomRowCount, 12);
  ...
  assert.equal(report.textMaterializationHeadroomRows.length, 12);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 156
    && entry.boundedFullStringRowCount === 147
    ...
    && entry.demandDrivenRows === 156
    && entry.directReadableStreamRows === 0
    && entry.notFullArrayBufferRows === 156
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Scanned artifacts: 166/);
  assert.match(markdown, /Measured rows recognized: 861/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 284/);
  assert.match(markdown, /Partial\/projection threshold rows: 37/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 206/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 156 \| 156 \| 147 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 156 \| 0 \| 0 \| 156 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:156\/156/);
});
