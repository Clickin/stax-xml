...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.scannedArtifactCount, 191);
  ...
  assert.equal(report.summary.measuredRowCount, 1039);
  assert.equal(report.summary.aggregateRowCount, 122);
  assert.equal(report.summary.largeJsFullRowCount, 693);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 100);
  assert.equal(report.summary.sourceModeRowCount, 441);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 334);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 242
    && entry.boundedFullStringRowCount === 233
    ...
    && entry.demandDrivenRows === 242
    && entry.directReadableStreamRows === 0
    && entry.notFullArrayBufferRows === 242
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Scanned artifacts: 191/);
  assert.match(markdown, /Measured rows recognized: 1039/);
  assert.match(markdown, /Aggregate rows recognized: 122/);
  assert.match(markdown, /1 GiB\+ JS full-string aggregate rows recognized: 100/);
  assert.match(markdown, /1 GiB\+ JS full-string rows recognized: 693/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 441/);
  assert.match(markdown, /Partial\/projection threshold rows: 35/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 334/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 242 \| 242 \| 233 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 242 \| 0 \| 0 \| 242 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:242\/242, file-backed-sync-iterable-byte-batches:53\/53, async-iterable-byte-batches:15\/15/);
