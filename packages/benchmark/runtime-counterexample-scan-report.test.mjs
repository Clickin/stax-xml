...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.ok(report.scannedArtifacts.includes('firefox-spidermonkey-taskcluster-debug-browser-diagnostic-dump-audit.json'));
  assert.ok(report.scannedArtifacts.includes('firefox-spidermonkey-taskcluster-debug-browser-launch-preflight-audit.json'));
  ...
  assert.ok(report.scannedArtifacts.includes('memory-frontier-audit.json'));
  assert.ok(report.scannedArtifacts.includes('target-distance-audit.json'));
  assert.ok(report.scannedArtifacts.includes('text-materialization-boundary-audit.json'));
  assert.ok(report.scannedArtifacts.includes('runtime-proof-handoff-validation.json'));
  ...
  assert.ok(report.scannedArtifacts.includes('text-folding-cost-candidate.json'));
  assert.ok(report.scannedArtifacts.includes('instrumentation-overhead-candidate.json'));
  assert.ok(report.scannedArtifacts.includes('no-counter-fold-trim-candidate.json'));
  assert.ok(report.scannedArtifacts.includes('no-counter-value-cache-candidate.json'));
  assert.ok(report.scannedArtifacts.includes('no-counter-name-fold-cache-candidate.json'));
  assert.ok(report.scannedArtifacts.includes('no-counter-string-fold-cache-candidate.json'));
  assert.ok(report.scannedArtifacts.includes('no-counter-name-fold-cache-fold-trim-candidate.json'));
  assert.ok(report.scannedArtifacts.includes('no-counter-name-fold-cache-cross-process-books-corpus.json'));
  assert.ok(report.scannedArtifacts.includes('no-counter-materialization-candidate.json'));
  assert.ok(report.scannedArtifacts.includes('no-counter-materialization-batch1-candidate.json'));
  assert.ok(report.scannedArtifacts.includes('no-counter-materialization-batch1-cross-runtime-books-corpus.json'));
  assert.ok(report.scannedArtifacts.includes('warmup-full-cross-process-books-corpus.json'));
  assert.ok(report.scannedArtifacts.includes('no-counter-medium-ascii-text-cross-process-books-corpus-warmup.json'));
  assert.ok(report.scannedArtifacts.includes('text-trim-cost-cross-process-books-corpus.json'));
  assert.ok(report.scannedArtifacts.includes('text-trim-cost-cross-process-diverse-cycle.json'));
  assert.ok(report.scannedArtifacts.includes('deno-candidate-headroom-cross-process-books-corpus.json'));
  assert.ok(report.scannedArtifacts.includes('deno-candidate-headroom-cross-process-midsize-corpus.json'));
  assert.ok(report.scannedArtifacts.includes('access-shape-rerun-cross-process-books-corpus.json'));
  ...
  assert.equal(report.summary.sourceModeRowCount, 642);
  ...
  assert.equal(report.summary.fastestLargeFullAggregateRowWithMemoryProof.sourceArtifact, 'access-shape-candidate-cross-process-batch8.json');
  assert.equal(report.summary.fastestLargeFullAggregateRowWithMemoryProof.runtimeLabel, 'Node/V8');
  assert.equal(report.summary.fastestLargeFullAggregateRowWithMemoryProof.id, 'rawFrameNameId');
  assert.equal(report.summary.fastestLargeFullAggregateRowWithMemoryProof.mibPerSec, 172.66);
  assert.equal(report.summary.fastestLargeFullAggregateRowWithMemoryProof.sampleCount, 3);
  assert.equal(report.summary.fastestLargeFullAggregateRowWithMemoryProof.spreadPercent, 3.03);
  ...
  assert.ok(report.scannedArtifacts.includes('deno-v8-codegen-trace-midsize-corpus.json'));
  ...
  assert.ok(report.aggregateRows.some(row =>
    ...
    && row.id === 'rawFrameNameIdNoCounters'
    && row.mibPerSec === 93.96
    && row.fullStringParity === true
    && row.boundedMemory === true
    && row.sampleCount === 3
    && row.spreadPercent === 2.88
  ...
  assert.ok(report.sourceModeRows.some(row =>
    ...
    && row.runtimeLabel === 'Node/V8'
    && row.id === 'rawFrameNameIdNoCountersStringFoldCache'
  ...
  assert.ok(report.sourceModeRows.some(row =>
    ...
    && row.runtimeLabel === 'Node/V8'
    && row.id === 'rawFrameNameIdNoCountersNameFoldCache'
    && row.mibPerSec === 94.71
    && row.fullStringParity === true
    && row.boundedMemory === true
    && row.sourceMode === 'generated-sync-iterable-byte-batches'
  ...
  assert.ok(report.sourceModeRows.some(row =>
    row.sourceArtifact === 'segment-tokenizer-full-checksum-candidate.json'
    && row.id === 'allTokenStringsNameCachedDocumentEventsNoObjects'
    && row.mibPerSec === 52.73
    && row.eventCount === 61236571
    && row.checksum === -716099804
    && row.fullStringParity === true
  ...
  assert.ok(report.sourceModeRows.some(row =>
    ...
    && row.id === 'public-event-object-stable-shape'
    && row.runtimeLabel === 'Node/V8'
    && row.sourceMode === 'file-backed-sync-iterable-byte-batches'
    && row.mibPerSec === 62.32
    && row.fullStringParity === true
  ...
  assert.ok(report.sourceModeRows.some(row =>
    ...
    && row.id === 'fetchReadableStreamFull'
    && row.runtimeLabel === 'Chrome/V8'
    && row.jsRuntime === true
  ...
  assert.ok(report.sourceModeRows.some(row =>
    row.sourceArtifact === 'browser-fetch-readable-stream-books-corpus.json'
    && row.id === 'fetchAsyncByteBatchFull'
    && row.runtimeLabel === 'Chrome/V8'
    && row.jsRuntime === true
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 4
    && entry.fastestMiBPerSec === 76.22
    && entry.fastestRow.sourceArtifact === 'stream-source-consumption-shapes.json'
    && entry.fastestRow.id === 'sync-iterable-byte-batches'
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.demandDrivenRows === 15
    && entry.directReadableStreamRows === 0
    && entry.backpressureRows === 15
    && entry.notFullArrayBufferRows === 15
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    entry.sourceMode === 'generated-sync-iterable-byte-batches'
    && entry.rowCount === 382
    && entry.boundedFullStringRowCount === 373
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 53
    && entry.boundedFullStringRowCount === 52
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 2
    && entry.boundedFullStringRowCount === 2
    && entry.fastestMiBPerSec === 14.64
    && entry.fastestRow.id === 'fetchReadableStreamFull'
    && entry.demandDrivenRows === 2
    && entry.directReadableStreamRows === 2
    && entry.backpressureRows === 2
    && entry.notFullArrayBufferRows === 2
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    entry.sourceMode === 'fetch-async-iterable-byte-batches'
    && entry.rowCount === 2
    && entry.boundedFullStringRowCount === 2
    && entry.fastestMiBPerSec === 13.52
    && entry.fastestRow.id === 'fetchAsyncByteBatchFull'
    && entry.demandDrivenRows === 2
    && entry.directReadableStreamRows === 0
    && entry.backpressureRows === 2
    && entry.notFullArrayBufferRows === 2
  ...
  assert.match(markdown, /Scanned artifacts: 228/);
  ...
  assert.match(markdown, /Rows with unknown bounded-memory flag: 25/);
  assert.match(markdown, /Rows with recognized source mode: 642/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 382 \| 382 \| 373 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 382 \| 0 \| 0 \| 382 \|/);
  assert.match(markdown, /\| `file-backed-sync-iterable-byte-batches` \| 53 \| 53 \| 52 \| 152\.11 \| Node\/V8 stax-raw-frame-name-id stax-raw-frame-name-id-batch-8 from file-backed-batch-size-sweep\.json \| 53 \| 0 \| 0 \| 53 \|/);
  assert.match(markdown, /\| `complete-js-string` \| 1 \| 1 \| 0 \| \d+\.\d{2} \| Bun\/JSC 3 from bun-event-reader-string-large\.json \| 0 \| 0 \| 0 \| 1 \|/);
  assert.match(markdown, /\| `async-iterable-byte-batches` \| 15 \| 15 \| 15 \| 77\.56 \| Node\/V8 async-iterable-raw-frame-ascii-batch-8 from stream-source-consumption-backpressure-counters\.json \| 15 \| 0 \| 15 \| 15 \|/);
  assert.match(markdown, /\| `web-readable-stream-pull` \| 15 \| 15 \| 15 \| 77\.86 \| Node\/V8 web-readable-stream-raw-frame-ascii-batch-8 from stream-source-consumption-shapes\.json \| 15 \| 15 \| 15 \| 15 \|/);
  assert.match(markdown, /\| `fetch-readable-stream-pull` \| 2 \| 2 \| 2 \| 14\.64 \| Chrome\/V8 fetchReadableStreamFull from browser-candidate-headroom-books-corpus\.json \| 2 \| 2 \| 2 \| 2 \|/);
  assert.match(markdown, /\| `fetch-async-iterable-byte-batches` \| 2 \| 2 \| 2 \| 13\.52 \| Chrome\/V8 fetchAsyncByteBatchFull from browser-candidate-headroom-books-corpus\.json \| 2 \| 0 \| 2 \| 2 \|/);
  assert.match(markdown, /\| `sync-iterable-byte-batches` \| 4 \| 4 \| 4 \| 76\.22 \| Node\/V8 sync-iterable-byte-batches from stream-source-consumption-shapes\.json \| 4 \| 0 \| 0 \| 4 \|/);
  assert.match(markdown, /Fastest 1 GiB\+ JS full-string aggregate row with memory proof: Node\/V8 rawFrameNameId from access-shape-candidate-cross-process-batch8\.json at avg 172\.66 MiB\/s/);
  ...
  assert.match(markdown, /async-iterable-byte-batches:15/);
  assert.match(markdown, /web-readable-stream-pull:15/);
  assert.match(markdown, /sync-iterable-byte-batches:4/);
  assert.match(markdown, /fetch-readable-stream-pull:2/);
...
test('runtime counterexample scan recognizes browser environment independent of artifact name', () => {
  const syntheticDir = join(tmpDir, 'browser-env-counterexample');
  const syntheticJsonOut = join(tmpDir, 'browser-env-counterexample.json');
  const syntheticMdOut = join(tmpDir, 'browser-env-counterexample.md');
  resetTmp();
  mkdirSync(syntheticDir, { recursive: true });
  writeFileSync(join(syntheticDir, 'external-run-result.json'), `${JSON.stringify({
    ...
    rows: [
      {
        id: 'externalSafariFullString',
        mibPerSec: 205,
        fullStringParity: true,
        boundedMemory: true,
        maxJsHeapUsedBytes: 90 * 1024 * 1024,
        eventCount: 1,
        checksum: 1,
        contractScope: 'full-string-checksum',
      },
  ...
  const report = JSON.parse(readFileSync(syntheticJsonOut, 'utf8'));
  assert.equal(report.summary.counterexampleCount, 1);
  assert.equal(report.counterexamples[0].sourceArtifact, 'external-run-result.json');
  assert.equal(report.counterexamples[0].runtimeLabel, 'Safari/JavaScriptCore');
  assert.equal(report.counterexamples[0].id, 'externalSafariFullString');
