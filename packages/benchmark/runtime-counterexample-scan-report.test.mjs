...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.scannedArtifactCount, 192);
  ...
  assert.equal(report.summary.measuredRowCount, 1057);
  assert.equal(report.summary.aggregateRowCount, 128);
  assert.equal(report.summary.largeJsFullRowCount, 705);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 104);
  assert.equal(report.summary.sourceModeRowCount, 459);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 346);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 254
    && entry.boundedFullStringRowCount === 245
    ...
    && entry.demandDrivenRows === 254
    && entry.directReadableStreamRows === 0
    && entry.notFullArrayBufferRows === 254
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Scanned artifacts: 192/);
  assert.match(markdown, /Measured rows recognized: 1057/);
  assert.match(markdown, /Aggregate rows recognized: 128/);
  assert.match(markdown, /1 GiB\+ JS full-string aggregate rows recognized: 104/);
  assert.match(markdown, /1 GiB\+ JS full-string rows recognized: 705/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 459/);
  assert.match(markdown, /Partial\/projection threshold rows: 35/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 346/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 254 \| 254 \| 245 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 254 \| 0 \| 0 \| 254 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:254\/254, file-backed-sync-iterable-byte-batches:53\/53, async-iterable-byte-batches:15\/15/);
