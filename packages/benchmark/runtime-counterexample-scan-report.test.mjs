...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.scannedArtifactCount, 151);
  ...
  assert.equal(report.summary.measuredRowCount, 802);
  assert.equal(report.summary.aggregateRowCount, 89);
  assert.equal(report.summary.largeJsFullRowCount, 496);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 69);
  assert.equal(report.summary.sourceModeRowCount, 225);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 166);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 118
    && entry.boundedFullStringRowCount === 109
    ...
    && entry.demandDrivenRows === 118
    && entry.notFullArrayBufferRows === 118
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Rows with recognized source mode: 225/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 166/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 118 \| 118 \| 109 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 118 \| 0 \| 118 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:118\/118/);
});
