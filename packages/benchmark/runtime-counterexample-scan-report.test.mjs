...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.sourceModeRowCount, 376);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 282);
  ...
  assert.match(markdown, /Measured rows recognized: 991/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 376/);
  assert.match(markdown, /Partial\/projection threshold rows: 38/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 282/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:196\/196, file-backed-sync-iterable-byte-batches:53\/53, async-iterable-byte-batches:13\/13/);
