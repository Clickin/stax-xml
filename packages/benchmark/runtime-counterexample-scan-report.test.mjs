...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.measuredRowCount, 907);
  assert.equal(report.summary.aggregateRowCount, 99);
  assert.equal(report.summary.largeJsFullRowCount, 566);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 77);
  assert.equal(report.summary.sourceModeRowCount, 328);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 236);
  ...
  assert.match(markdown, /Measured rows recognized: 907/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 328/);
  assert.match(markdown, /Partial\/projection threshold rows: 38/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 236/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:179\/179, file-backed-sync-iterable-byte-batches:49\/49/);
});
