...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.scannedArtifactCount, 165);
  ...
  assert.equal(report.summary.measuredRowCount, 857);
  assert.equal(report.summary.aggregateRowCount, 93);
  assert.equal(report.summary.largeJsFullRowCount, 533);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 73);
  assert.equal(report.summary.sourceModeRowCount, 280);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 203);
  assert.equal(report.summary.partialHeadroomRowCount, 36);
  assert.equal(report.summary.textMaterializationHeadroomRowCount, 11);
  ...
  assert.equal(report.textMaterializationHeadroomRows.length, 11);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 153
    && entry.boundedFullStringRowCount === 144
    ...
    && entry.demandDrivenRows === 153
    && entry.directReadableStreamRows === 0
    && entry.notFullArrayBufferRows === 153
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Scanned artifacts: 165/);
  assert.match(markdown, /Measured rows recognized: 857/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 280/);
  assert.match(markdown, /Partial\/projection threshold rows: 36/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 203/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 153 \| 153 \| 144 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 153 \| 0 \| 0 \| 153 \|/);
  ...
  assert.match(markdown, /\| `web-readable-stream-pull` \| 1 \| 1 \| 1 \| 111\.56 \| Node\/V8 web-readable-stream-pull from stream-source-consumption-shapes\.json \| 1 \| 1 \| 1 \| 1 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:153\/153/);
});
