...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.sourceModeRowCount, 380);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 286);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 196
    && entry.boundedFullStringRowCount === 187
    ...
    && entry.demandDrivenRows === 196
    && entry.directReadableStreamRows === 0
    && entry.notFullArrayBufferRows === 196
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Measured rows recognized: 995/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 380/);
  assert.match(markdown, /Partial\/projection threshold rows: 38/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 286/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 196 \| 196 \| 187 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 196 \| 0 \| 0 \| 196 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:196\/196, file-backed-sync-iterable-byte-batches:53\/53, async-iterable-byte-batches:15\/15/);
