...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.scannedArtifactCount, 215);
  ...
  assert.equal(report.summary.measuredRowCount, 1209);
  assert.equal(report.summary.aggregateRowCount, 170);
  assert.equal(report.summary.largeJsFullRowCount, 814);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 133);
  assert.equal(report.summary.sourceModeRowCount, 596);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 440);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 348
    && entry.boundedFullStringRowCount === 339
    ...
    && entry.demandDrivenRows === 348
    && entry.directReadableStreamRows === 0
    && entry.notFullArrayBufferRows === 348
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Scanned artifacts: 215/);
  assert.match(markdown, /Measured rows recognized: 1209/);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string rows recognized: 814/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 596/);
  assert.match(markdown, /Partial\/projection threshold rows: 35/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 440/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 348 \| 348 \| 339 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 348 \| 0 \| 0 \| 348 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:348\/348, file-backed-sync-iterable-byte-batches:53\/53, async-iterable-byte-batches:15\/15/);
