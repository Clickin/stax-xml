...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.scannedArtifactCount, 187);
  ...
  assert.equal(report.summary.measuredRowCount, 1016);
  assert.equal(report.summary.aggregateRowCount, 117);
  assert.equal(report.summary.largeJsFullRowCount, 670);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 95);
  assert.equal(report.summary.sourceModeRowCount, 418);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 311);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 219
    && entry.boundedFullStringRowCount === 210
    ...
    && entry.demandDrivenRows === 219
    && entry.directReadableStreamRows === 0
    && entry.notFullArrayBufferRows === 219
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Scanned artifacts: 187/);
  assert.match(markdown, /Measured rows recognized: 1016/);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string rows recognized: 670/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 418/);
  assert.match(markdown, /Partial\/projection threshold rows: 35/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 311/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 219 \| 219 \| 210 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 219 \| 0 \| 0 \| 219 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:219\/219, file-backed-sync-iterable-byte-batches:53\/53, async-iterable-byte-batches:15\/15/);
