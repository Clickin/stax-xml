...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.scannedArtifactCount, 150);
  ...
  assert.equal(report.summary.measuredRowCount, 800);
  assert.equal(report.summary.aggregateRowCount, 89);
  assert.equal(report.summary.largeJsFullRowCount, 494);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 69);
  assert.equal(report.summary.sourceModeRowCount, 223);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 164);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 116
    && entry.boundedFullStringRowCount === 107
    ...
    && entry.demandDrivenRows === 116
    && entry.notFullArrayBufferRows === 116
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Rows with recognized source mode: 223/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 164/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 116 \| 116 \| 107 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 116 \| 0 \| 116 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:116\/116/);
});
