...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.measuredRowCount, 927);
  assert.equal(report.summary.aggregateRowCount, 99);
  assert.equal(report.summary.largeJsFullRowCount, 586);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 77);
  assert.equal(report.summary.sourceModeRowCount, 348);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 256);
  ...
  assert.match(markdown, /Measured rows recognized: 927/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 348/);
  assert.match(markdown, /Partial\/projection threshold rows: 38/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 256/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:179\/179, file-backed-sync-iterable-byte-batches:49\/49, async-iterable-byte-batches:12\/12/);
