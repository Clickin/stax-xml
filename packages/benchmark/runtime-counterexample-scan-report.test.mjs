...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.scannedArtifactCount, 186);
  ...
  assert.equal(report.summary.measuredRowCount, 1013);
  assert.equal(report.summary.aggregateRowCount, 117);
  assert.equal(report.summary.largeJsFullRowCount, 667);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 95);
  assert.equal(report.summary.sourceModeRowCount, 415);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 308);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 216
    && entry.boundedFullStringRowCount === 207
    ...
    && entry.demandDrivenRows === 216
    && entry.directReadableStreamRows === 0
    && entry.notFullArrayBufferRows === 216
    && entry.fullArrayBufferRows === 0
    && entry.unknownArrayBufferRows === 0
  ));
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
  ...
  assert.match(markdown, /Scanned artifacts: 186/);
  assert.match(markdown, /Measured rows recognized: 1013/);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string rows recognized: 667/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 415/);
  assert.match(markdown, /Partial\/projection threshold rows: 35/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 308/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 216 \| 216 \| 207 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 216 \| 0 \| 0 \| 216 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:216\/216, file-backed-sync-iterable-byte-batches:53\/53, async-iterable-byte-batches:15\/15/);
