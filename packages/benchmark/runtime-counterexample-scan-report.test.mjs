...
test('runtime counterexample scan applies the broad 200 MiB/s rule mechanically', () => {
  ...
  assert.equal(report.summary.measuredRowCount, 1201);
  assert.equal(report.summary.aggregateRowCount, 170);
  assert.equal(report.summary.largeJsFullRowCount, 807);
  assert.equal(report.summary.largeJsFullAggregateRowCount, 133);
  assert.equal(report.summary.sourceModeRowCount, 588);
  assert.equal(report.summary.largeJsFullSourceModeRowCount, 433);
  ...
  assert.ok(report.summary.largeJsFullSourceModeBreakdown.some(entry =>
    ...
    && entry.rowCount === 341
    && entry.boundedFullStringRowCount === 332
  ...
  assert.match(markdown, /Scanned artifacts: 214/);
  assert.match(markdown, /Measured rows recognized: 1201/);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string rows recognized: 807/);
  ...
  assert.match(markdown, /Rows with recognized source mode: 588/);
  assert.match(markdown, /Partial\/projection threshold rows: 35/);
  assert.match(markdown, /1 GiB\+ JS full-string rows with recognized source mode: 433/);
  ...
  assert.match(markdown, /\| `generated-sync-iterable-byte-batches` \| 341 \| 341 \| 332 \| 185\.50 \| Node\/V8 rawFrameNameId from text-trim-cost-decomposition\.json \| 341 \| 0 \| 0 \| 341 \|/);
  ...
  assert.match(markdown, /not-full-ArrayBuffer parser-input rows are generated-sync-iterable-byte-batches:341\/341, file-backed-sync-iterable-byte-batches:53\/53, async-iterable-byte-batches:15\/15/);
