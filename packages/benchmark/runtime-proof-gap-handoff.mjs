    ...
    items.push({
      ...
      blockers: [
        ...
        materializedScopeDistancePinned
          ? `The materialized scope-distance audit pins semanticEquivalentForAsciiFields=true while closureRequirementsMet=${materializedScopeDistance.summary?.closureRequirementsMet ?? 'unknown'} and closureRequirementsBlocked=${materializedScopeDistance.summary?.closureRequirementsBlocked ?? 'unknown'}; primarySyncByteBatchMissingGlobals=${(materializedScopeDistance.hostApiSurface?.primarySyncByteBatchMissingGlobals ?? []).join(', ') || 'unknown'}; closesCodegenObligation=false, preventing the materialized js-shell artifact from being cited as unchanged StAX closure evidence.`
