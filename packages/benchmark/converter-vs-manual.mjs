...
async function parseManual(xmlStream) {
    const books = [];
...
function parseManualSync(xmlString) {
    const books = [];
    let currentBook = null;
    let currentChapters = [];
    const elementStack = [];
...
async function parseConverter(xmlStream) {
...
function parseConverterSync(xmlString) {
...
if (manualResult.length > 0 && converterResult.length > 0) {
    ...
    const fieldsMatch = manualBook.id === converterBook.id &&
