...
// Book interface for JSON output
interface Book {
  id: string;
  title: string;
  author: string;
  ...
  publishDate: string;
  description: string;
  chapters: Array<{ number: string; text: string }>;
}
...
async function parseSAX(xmlStream: ReadableStream<Uint8Array>): Promise<Book[]> {
...
async function parseStaxXML(xmlStream: ReadableStream<Uint8Array>): Promise<Book[]> {
  const books: Book[] = [];
  let currentBook: Partial<Book> | null = null;
  let currentChapters: Array<{ number: string; text: string }> = [];
  const elementStack: string[] = []; // Track element hierarchy
