...
function parseTools(value) {
  ...
  for (const tool of tools) {
    if (!allTools.includes(tool) && !optInTools.includes(tool)) {
...
function consumeStaxRawFrameNameId(bytes, options = {}) {
  ...
  const shortAttrValueCache = options.shortAttrValueCache === true ? new ShortSpanStringCache(4) : undefined;
  let eventCount = 0;
  let checksum = 0;
  let frame;
      ...
      if (type === StreamEventType.CHARACTERS || type === StreamEventType.CDATA) {
        ...
        if (start >= 0) {
          const value = options.longAsciiText
            ? materializeLongAsciiTextValue(buffer, start, textEnds[index], decoder, valueCache)
            : materializeValue(buffer, start, textEnds[index], decoder, valueCache);
...
function materializeLongAsciiTextValue(buffer, start, end, decoder, valueCache) {
  if (!valueCache) {
    return decodeLongAsciiTextSpan(buffer, start, end, decoder);
  }
  const cached = valueCache.get(buffer, start, end);
  if (cached !== undefined) {
    return cached;
  }
  const value = decodeLongAsciiTextSpan(buffer, start, end, decoder);
  valueCache.set(buffer, start, end, value);
  return value;
}

function materializeAttrValue(buffer, start, end, decoder, valueCache, shortAttrValueCache) {
  if (shortAttrValueCache) {
    const cached = shortAttrValueCache.get(buffer, start, end);
    if (cached !== undefined) {
      return cached;
    }
    const value = decodeSpan(buffer, start, end, decoder);
    shortAttrValueCache.set(buffer, start, end, value);
    return value;
  }
  return materializeValue(buffer, start, end, decoder, valueCache);
}

class ShortSpanStringCache {
  ...
  get(buffer, start, end) {
    const key = shortSpanKey(buffer, start, end, this.maxLength);
...
function shortSpanKey(buffer, start, end, maxLength) {
  const length = end - start;
  if (length < 0 || length > maxLength || length > 4) {
    return undefined;
  }
  let key = length << 24;
  for (let index = 0; index < length; index++) {
    key |= buffer[start + index] << (index * 8);
  }
  return key >>> 0;
}
...
function decodeLongAsciiTextSpan(buffer, start, end, decoder) {
  const ascii = decodeShortAsciiSpan(buffer, start, end);
  if (ascii !== undefined) {
    return ascii;
  }
...
async function main() {
  ...
  const needsPreloadedStaxStream = (
    ...
    || options.tools.includes('stax-raw-frame-name-id-long-ascii-text')
