...
function createWebStream(fixturePath) {
    return Readable.toWeb(createReadStream(fixturePath));
}
async function measure(createParser, warmups, runs) {
...
async function main(argv = process.argv.slice(2)) {
    ...
    const current = await import('stax-xml');
