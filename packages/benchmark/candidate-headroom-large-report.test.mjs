...
test('large candidate headroom matrix preserves bounded byte-batch contract', () => {
  ...
  assert.deepEqual(report.variants.map(entry => entry.id), [
    ...
    'rawFrameNameIdUnrolledMediumAsciiTextTrimGuard',
  ...
  const rawNameIdUnrolledMediumAsciiText = report.variants.find(entry => entry.id === 'rawFrameNameIdUnrolledMediumAsciiText');
  const rawNameIdUnrolledMediumAsciiTextTrimGuard = report.variants.find(entry => entry.id === 'rawFrameNameIdUnrolledMediumAsciiTextTrimGuard');
  const rawNameIdMediumAsciiAttrValue = report.variants.find(entry => entry.id === 'rawFrameNameIdMediumAsciiAttrValue');
  const rawNameIdAttrValueCache = report.variants.find(entry => entry.id === 'rawFrameNameIdAttrValueCache');
  const rawNameIdTextCache = report.variants.find(entry => entry.id === 'rawFrameNameIdTextCache');
  const rawNameIdOffsetTextCache = report.variants.find(entry => entry.id === 'rawFrameNameIdOffsetTextCache');
  ...
  const rawNameIdAllAsciiSpans = report.variants.find(entry => entry.id === 'rawFrameNameIdAllAsciiSpans');
  ...
  assert.equal(rawNameIdMediumAsciiText.fullStringParity, true);
  assert.equal(rawNameIdMediumAsciiText.checksum, report.fullStringParity.checksum);
  assert.equal(rawNameIdMediumAsciiText.materializationCounters.stringFieldReads, rawNameId.materializationCounters.stringFieldReads);
  ...
  assert.equal(rawNameIdUnrolledMediumAsciiText.fullStringParity, true);
  assert.equal(rawNameIdUnrolledMediumAsciiText.checksum, report.fullStringParity.checksum);
  assert.equal(rawNameIdUnrolledMediumAsciiText.materializationCounters.stringFieldReads, rawNameId.materializationCounters.stringFieldReads);
  assert.equal(rawNameIdUnrolledMediumAsciiText.materializationCounters.rawSpanMaterializations, rawNameId.materializationCounters.rawSpanMaterializations);
  assert.equal(typeof rawNameIdUnrolledMediumAsciiText.materializationCounters.unrolledMediumAsciiTextHits, 'number');
  assert.equal(typeof rawNameIdUnrolledMediumAsciiText.materializationCounters.unrolledMediumAsciiTextFallbacks, 'number');
  assert.equal(rawNameIdUnrolledMediumAsciiTextTrimGuard.fullStringParity, true);
  assert.equal(rawNameIdUnrolledMediumAsciiTextTrimGuard.checksum, report.fullStringParity.checksum);
  assert.equal(rawNameIdUnrolledMediumAsciiTextTrimGuard.materializationCounters.stringFieldReads, rawNameId.materializationCounters.stringFieldReads);
  assert.equal(rawNameIdUnrolledMediumAsciiTextTrimGuard.materializationCounters.rawSpanMaterializations, rawNameId.materializationCounters.rawSpanMaterializations);
  assert.equal(typeof rawNameIdUnrolledMediumAsciiTextTrimGuard.materializationCounters.unrolledMediumAsciiTextHits, 'number');
  assert.equal(typeof rawNameIdUnrolledMediumAsciiTextTrimGuard.materializationCounters.unrolledMediumAsciiTextFallbacks, 'number');
  assert.equal(typeof rawNameIdUnrolledMediumAsciiTextTrimGuard.materializationCounters.textTrimGuardSkips, 'number');
  assert.equal(typeof rawNameIdUnrolledMediumAsciiTextTrimGuard.materializationCounters.textTrimGuardFallbacks, 'number');
  ...
  assert.equal(rawNameIdAttrValueCache.fullStringParity, true);
  assert.equal(rawNameIdAttrValueCache.checksum, report.fullStringParity.checksum);
  assert.equal(rawNameIdAttrValueCache.materializationCounters.stringFieldReads, rawNameId.materializationCounters.stringFieldReads);
  assert.equal(typeof rawNameIdAttrValueCache.materializationCounters.rawValueCacheHits, 'number');
  assert.equal(typeof rawNameIdAttrValueCache.materializationCounters.rawValueCacheMisses, 'number');
  ...
  assert.equal(rawNameIdOffsetTextCache.fullStringParity, true);
  assert.equal(rawNameIdOffsetTextCache.checksum, report.fullStringParity.checksum);
  assert.equal(rawNameIdOffsetTextCache.materializationCounters.stringFieldReads, rawNameId.materializationCounters.stringFieldReads);
  assert.equal(typeof rawNameIdOffsetTextCache.materializationCounters.rawValueCacheHits, 'number');
  assert.equal(typeof rawNameIdOffsetTextCache.materializationCounters.rawValueCacheMisses, 'number');
  ...
  assert.equal(rawNameIdAllAsciiSpans.fullStringParity, true);
  assert.equal(rawNameIdAllAsciiSpans.checksum, report.fullStringParity.checksum);
  assert.equal(rawNameIdAllAsciiSpans.materializationCounters.stringFieldReads, rawNameId.materializationCounters.stringFieldReads);
  assert.ok(rawNameIdAllAsciiSpans.materializationCounters.asciiSpanMaterializationHits > 0);
  assert.ok(rawNameIdAllAsciiSpans.materializationCounters.asciiSpanMaterializationFallbacks > 0);
  ...
  assert.ok(report.findings.some(entry => entry.id === 'all-ascii-span-materialization-candidate'));
...
test('large candidate headroom matrix can isolate materialization counter overhead', () => {
  ...
  const result = spawnSync(process.execPath, [
    ...
    '0',
    '--cases',
    'rawFrameNameId,rawFrameNameIdNoCounters,rawFrameNameIdNoCountersFoldTrim,rawFrameNameIdNoCountersTextCache,rawFrameNameIdNoCountersValueCache,rawFrameNameIdNoCountersNameFoldCache',
    '--json-out',
    noCountersJsonOut,
    '--md-out',
    noCountersMdOut,
  ...
  for (const cacheRow of [noCountersTextCache, noCountersValueCache, noCountersNameFoldCache]) {
