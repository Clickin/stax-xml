 ...
 */
export function calculateDescriptiveStats(sample: number[]): DescriptiveStats {
  if (sample.length === 0) {
 ...
 */
export function detectOutliers(sample: number[]): {
  outliers: number[];
  cleaned: number[];
  lowerBound: number;
  upperBound: number;
 ...
 */
export function cohensD(sample1: number[], sample2: number[]): {
  d: number;
 ...
 */
export function formatStatisticalReport(
  baseline: number[],
  optimized: number[],
  baselineName: string = 'Baseline',
  optimizedName: string = 'Optimized'
): string {
