...
import { HeapProfiler } from './memory-tracker.js';
...
async function exampleStringConcatenation() {
  ...
  const concatResult = await benchmarkWithGC(
  ...
  const joinResult = await benchmarkWithGC(
    'Array Join',
    () => {
      const parts: string[] = [];
      for (let i = 0; i < numStrings; i++) {
...
async function exampleGCDetails() {
  ...
  result.gcMetrics.gcEvents.forEach((event, idx) => {
    console.log(
      `  ${idx + 1}. ${event.kind.padEnd(12)} @ ${event.timestamp.toFixed(2)}ms (${event.duration.toFixed(2)}ms)`
    );
  });
