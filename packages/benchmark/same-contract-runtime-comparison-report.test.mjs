...
test('same-contract runtime comparison aggregates existing rows without normalizing memory models', () => {
  ...
  assert.equal(report.summary.textMaterializationFrontier.negativeCandidateCount, 35);
  ...
  assert.equal(report.summary.memoryFrontier.rows, 239);
  assert.equal(report.summary.memoryFrontier.boundedRows, 222);
  ...
  assert.ok(report.metadata.sourceArtifacts.includes('no-counter-materialization-batch1-cross-runtime-books-corpus.json'));
  ...
  assert.ok(report.comparisonRows.some(row =>
    row.group === 'no-counter-materialization-batch1-cross-runtime-books-corpus'
  ...
  assert.match(markdown, /Text materialization frontier: fastest full row rawFrameNameId at 185\.50 MiB\/s, 14\.50 MiB\/s below 200 MiB\/s; without-text rows crossing target: 4; negative candidates: 35/);
