...
test('same-contract runtime comparison aggregates existing rows without normalizing memory models', () => {
  ...
  assert.equal(report.summary.rowCount, 261);
  ...
  assert.equal(report.summary.memoryFrontier.rows, 216);
  assert.equal(report.summary.memoryFrontier.boundedRows, 199);
  ...
  assert.deepEqual(report.summary.sourceShapeSafety, {
    ...
    sourceModeBreakdown: [
      ...
      },
      {
        ...
        rows: 172,
        notFullArrayBufferRows: 172,
        fullArrayBufferRows: 0,
        unknownArrayBufferRows: 0,
        directReadableStreamRows: 0,
        corpusSeedReplayRows: 125,
        fastestRow: {
  ...
  assert.equal(report.summary.sourceConsumptionFrontier.fastestSyncIterable.mibPerSec, 67.94);
  ...
  assert.equal(report.summary.sourceConsumptionFrontier.fastestAsyncIterable.mibPerSec, 76.9);
  assert.equal(report.summary.sourceConsumptionFrontier.fastestAsyncIterableRatioToFastestSyncIterable, 1.13);
  ...
  assert.equal(report.summary.sourceConsumptionFrontier.fastestReadableStream.mibPerSec, 75.98);
  ...
  assert.equal(report.summary.sourceConsumptionFrontier.fastestReadableStreamRatioToFastestSyncIterable, 1.12);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string memory frontier: 199\/216 bounded rows; fastest bounded row Node\/V8 rawFrameNameId at 185\.50 MiB\/s \(process RSS max 60\.45 MiB\)/);
  ...
  assert.match(markdown, /Source consumption frontier: sync byte batches sync-iterable-byte-batches-batch-8 at 67\.94 MiB\/s; direct ReadableStream web-readable-stream-raw-frame-ascii-batch-8 at 75\.98 MiB\/s \(1\.12x sync\); backpressure rows 6\/6/);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string source-mode rows not using full ArrayBuffer parser input: 210\/210/);
  assert.match(markdown, /1 GiB\+ source-mode rows replaying a corpus seed buffer: 127 \(max seed 100\.26 MiB, max seed\/target 0\.09\)/);
  assert.match(markdown, /\| 1 GiB\+ JS full-string rows with source mode metadata \| 210 \| 210 \| 0 \| 0 \| 36 \| 1 \| 127 \| 100\.26 MiB \|/);
  ...
  assert.match(markdown, /\| `sync-iterable-byte-batches` \| 172 \| 172 \| 0 \| 0 \| 0 \| 125 \| Node\/V8 `rawFrameNameId` 185\.50 MiB\/s from `text-trim-cost-decomposition\.json` \|/);
  ...
  assert.match(markdown, /\| Fastest sync byte batches \| `sync-iterable-byte-batches-batch-8` \| synchronous Iterable<Uint8Array\[\]> \| 67\.94 \| 239\.91 MiB \| 8 \| no \| no \| n\/a \| reads=16385, batches=2048, pulls=0, enqueues=0 \|/);
  assert.match(markdown, /\| Fastest direct ReadableStream \| `web-readable-stream-raw-frame-ascii-batch-8` \| Web ReadableStream<Uint8Array> \| 75\.98 \| 257\.61 MiB \| 8 \| yes \| no \| yes \| reads=16385, batches=0, pulls=16385, enqueues=16384 \|/);
  ...
  assert.match(markdown, /\| process-rss \| 187 \| 179 \| 8 \| 1956\.69 MiB \| Node\/V8 rawFrameNameId at 185\.50 MiB\/s/);
