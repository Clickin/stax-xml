...
test('same-contract runtime comparison aggregates existing rows without normalizing memory models', () => {
  ...
  assert.equal(report.summary.rowCount, 215);
  assert.equal(report.summary.jsLargeFullRowCount, 184);
  ...
  assert.equal(report.summary.memoryFrontier.rows, 184);
  assert.equal(report.summary.memoryFrontier.boundedRows, 167);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string memory frontier: 167\/184 bounded rows; fastest bounded row Node\/V8 rawFrameNameId at 185\.50 MiB\/s \(process RSS max 60\.45 MiB\)/);
  ...
  assert.match(markdown, /1 GiB\+ source-mode rows replaying a corpus seed buffer: 101 \(max seed 100\.26 MiB, max seed\/target 0\.09\)/);
  assert.match(markdown, /\| 1 GiB\+ JS full-string rows with source mode metadata \| 178 \| 178 \| 0 \| 0 \| 101 \| 100\.26 MiB \|/);
  ...
  assert.match(markdown, /Rows classified: 184/);
  ...
  assert.match(markdown, /\| process-rss \| 155 \| 147 \| 8 \| 1956\.69 MiB \| Node\/V8 rawFrameNameId at 185\.50 MiB\/s/);
