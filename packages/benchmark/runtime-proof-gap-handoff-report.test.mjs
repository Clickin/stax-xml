...
test('runtime proof gap handoff tracks current open coverage obligations', () => {
  ...
  assert.equal(report.sourceConsumptionEvidence.rowCount, 277);
  ...
  assert.equal(report.memoryFrontierEvidence.rows, 230);
  assert.equal(report.memoryFrontierEvidence.boundedRows, 213);
  ...
  assert.match(markdown, /\| sync-iterable-byte-batches \| 186 \| 186 \| 0 \| 139 \| Node\/V8 rawFrameNameId 185\.50 MiB\/s from text-trim-cost-decomposition\.json \|/);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string memory rows: 230/);
