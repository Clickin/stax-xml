...
test('runtime proof gap handoff tracks current open coverage obligations', () => {
  ...
  assert.equal(report.sourceConsumptionEvidence.rowCount, 221);
  ...
  assert.equal(report.memoryFrontierEvidence.rows, 188);
  assert.equal(report.memoryFrontierEvidence.boundedRows, 171);
  ...
  assert.equal(report.textMaterializationEvidence.negativeCandidateCount, 17);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string memory rows: 188/);
