...
test('runtime proof gap handoff tracks current open coverage obligations', () => {
  ...
  assert.ok(spiderMonkey.localClosure.blockers.some(item => /current Taskcluster debug js-shell emits JitSpew codegen output \(taskId=aJLr1DFjQ7urQTpRiIsfRQ, buildId=20260602093330, sourceRevision=[0-9a-f]{40}\), but sameContractStaxRow=false, canRunCurrentStaxFullStringBenchmark=false, and selectedRowIdentityStatus=not-claimed-non-stax-diagnostic/.test(item)));
  assert.ok(spiderMonkey.localClosure.blockers.some(item => /current Taskcluster debug js-shell emits JitSpew codegen output while running the XML byte-tokenizer workload \(taskId=aJLr1DFjQ7urQTpRiIsfRQ, buildId=20260602093330, sourceRevision=[0-9a-f]{40}\), but fullStringParity=false, sameContractStaxRow=false, canRunCurrentStaxFullStringBenchmark=false, and selectedRowIdentityStatus=not-claimed-non-stax-diagnostic/.test(item)));
  assert.ok(spiderMonkey.localClosure.blockers.some(item => /current Taskcluster debug js-shell emits JitSpew codegen output while materializing JS strings and public event-shaped objects \(taskId=aJLr1DFjQ7urQTpRiIsfRQ, buildId=20260602093330, sourceRevision=[0-9a-f]{40}\), but unchangedStaxBenchmark=false, sameContractStaxRow=false, canRunCurrentStaxFullStringBenchmark=false, and selectedRowIdentityStatus=not-claimed-non-stax-diagnostic/.test(item)));
  assert.ok(spiderMonkey.localClosure.blockers.some(item => /rerun of the current Taskcluster debug js-shell codegen probe reproduces JitSpew output \(taskId=aJLr1DFjQ7urQTpRiIsfRQ, buildId=20260602093330, sourceRevision=[0-9a-f]{40}, codegenMarkers=54756\), but sameContractStaxRow=false, canRunCurrentStaxFullStringBenchmark=false, and closesEmittedIrObligation=false/.test(item)));
  assert.ok(spiderMonkey.localClosure.blockers.some(item => /rerun of the current Taskcluster debug js-shell materialized string\/object probe reproduces JitSpew output \(taskId=aJLr1DFjQ7urQTpRiIsfRQ, buildId=20260602093330, sourceRevision=[0-9a-f]{40}, codegenMarkers=234522, throughputMiBPerSec=0\.31\)/.test(item)));
