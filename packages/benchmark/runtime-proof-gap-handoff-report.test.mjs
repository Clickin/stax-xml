...
test('runtime proof gap handoff tracks current open coverage obligations', () => {
  ...
  assert.equal(report.sourceConsumptionEvidence.rowCount, 229);
  ...
  assert.equal(report.memoryFrontierEvidence.rows, 196);
  assert.equal(report.memoryFrontierEvidence.boundedRows, 179);
  ...
  assert.match(markdown, /1 GiB\+ JS full-string memory rows: 196/);
