...
test('runtime proof gap handoff tracks current open coverage obligations', () => {
  ...
  assert.equal(report.textMaterializationEvidence.negativeCandidateCount, 38);
  ...
  assert.deepEqual(spiderMonkey.localClosure.evidenceArtifacts, [
    ...
    'firefox-spidermonkey-taskcluster-debug-browser-launch-preflight-audit.json',
    ...
    'spidermonkey-taskcluster-debug-jsshell-ascii-stax-codegen-audit.json',
  ...
  assert.ok(spiderMonkey.localClosure.blockers.some(item => /Taskcluster debug Firefox browser launch preflight failed before harness startup at FIREFOX_PATH=.*firefox\.exe \(attempts=2, dllBlocklistFailures=2, disableDllBlocklistChangedFailure=false\)/i.test(item)));
  ...
  assert.ok(spiderMonkey.localClosure.blockers.some(item => /StAX public reader host API boundary audit separates the primary UTF-8 byte-batch path from non-primary host globals.*primarySyncByteBatchRequiresTextDecoder=false.*utf8FallbackDecoder=true.*nonUtf8RequiresTextDecoder=true.*eventReaderSyncDocumentStringInputRequiresTextEncoder=true.*xmlObjectStringInputRequiresTextEncoder=true.*projectionCompileAndStringInputRequiresTextEncoder=true.*compiledConverterStringInputRequiresTextEncoder=true.*rootImportRequiresTextEncoder=false.*asyncWriterOutputRequiresTextEncoder=true.*syncWriterOutputRequiresTextEncoder=false.*TextEncoder is therefore a generated-fixture\/string-input\/async-output boundary, not a primary byte-batch reader materialization requirement/.test(item)));
  ...
  assert.ok(safari.evidenceIntakeContract.requiredArtifacts.includes('safari-webdriver-candidate-headroom-cross-process-books-corpus.json'));
  assert.ok(safari.evidenceIntakeContract.requiredArtifacts.includes('target-distance-audit.json'));
  assert.ok(safari.evidenceIntakeContract.requiredRowFields.includes('runtimeId=safari-jsc-browser'));
  assert.ok(safari.evidenceIntakeContract.requiredRowFields.includes('parserInput=synchronous Iterable<Uint8Array[]>'));
  assert.ok(safari.evidenceIntakeContract.requiredRowFields.includes('sizeGiB>=1'));
  assert.ok(safari.evidenceIntakeContract.requiredAuditFields.includes('coverage.safariWebKitStatus.closesSafariObligation=true'));
  assert.ok(safari.evidenceIntakeContract.rejectionRules.some(item => /full XML ArrayBuffer/.test(item)));
  assert.ok(safari.evidenceIntakeContract.rejectionRules.some(item => /runtime-counterexample-scan/.test(item)));
  ...
  assert.ok(spiderMonkey.evidenceIntakeContract.requiredArtifacts.includes('spidermonkey-emitted-codegen-or-optimized-code artifact'));
  assert.ok(spiderMonkey.evidenceIntakeContract.requiredArtifacts.includes('spidermonkey-codegen-closure-audit.json'));
  assert.ok(spiderMonkey.evidenceIntakeContract.requiredRowFields.includes('runtimeId=firefox-spidermonkey|spidermonkey-js-shell'));
  assert.ok(spiderMonkey.evidenceIntakeContract.requiredRowFields.includes('selectedRowId'));
  assert.ok(spiderMonkey.evidenceIntakeContract.requiredRowFields.includes('emittedDumpMetadata'));
  assert.ok(spiderMonkey.evidenceIntakeContract.requiredAuditFields.includes('coverage.spiderMonkeyDiagnostics.emittedIrEvidenceCount>0'));
  assert.ok(spiderMonkey.evidenceIntakeContract.requiredAuditFields.includes('spidermonkey-codegen-closure-audit.summary.qualifiedClosureCount>0'));
  assert.ok(spiderMonkey.evidenceIntakeContract.rejectionRules.some(item => /jit-status-only/.test(item)));
  assert.ok(spiderMonkey.evidenceIntakeContract.rejectionRules.some(item => /rerun-stability evidence/.test(item)));
  ...
  assert.match(markdown, /StAX public reader host API boundary audit separates the primary UTF-8 byte-batch path from non-primary host globals.*primarySyncByteBatchRequiresTextDecoder=false.*utf8FallbackDecoder=true.*nonUtf8RequiresTextDecoder=true.*eventReaderSyncDocumentStringInputRequiresTextEncoder=true.*xmlObjectStringInputRequiresTextEncoder=true.*projectionCompileAndStringInputRequiresTextEncoder=true.*compiledConverterStringInputRequiresTextEncoder=true.*rootImportRequiresTextEncoder=false.*asyncWriterOutputRequiresTextEncoder=true.*syncWriterOutputRequiresTextEncoder=false.*TextEncoder is therefore a generated-fixture\/string-input\/async-output boundary, not a primary byte-batch reader materialization requirement/);
  ...
  assert.match(markdown, /SpiderMonkey codegen closure audit checks 18 diagnostic\/codegen candidates/);
  ...
  assert.match(markdown, /evidenceClass jit-status-only, bytecode-diagnostic-only/);
