...
function createFrontierAuditSnapshot(memoryFrontier, targetDistance, textMaterializationBoundary, textMaterializationFrontierCoverage, comparison = null, counterexampleSnapshot = null) {
  ...
  const text = {
    ...
    fastestNoTrimBoundedMemory: textMaterializationBoundary?.summary?.fastestNoTrim?.boundedMemory ?? null,
    fastestNoTrimTextStringReads: textMaterializationBoundary?.summary?.fastestNoTrim?.textStringReads ?? null,
    fastestNoTrimStringFieldReads: textMaterializationBoundary?.summary?.fastestNoTrim?.stringFieldReads ?? null,
    fastestFoldTrimBoundedMemory: textMaterializationBoundary?.summary?.fastestFoldTrim?.boundedMemory ?? null,
    fastestFoldTrimTextStringReads: textMaterializationBoundary?.summary?.fastestFoldTrim?.textStringReads ?? null,
    fastestFoldTrimStringFieldReads: textMaterializationBoundary?.summary?.fastestFoldTrim?.stringFieldReads ?? null,
  ...
  return [
    ...
    },
    {
      ...
      satisfied: snapshot?.allRequiredFlagsPresent === true
        ...
        && snapshot.scriptsReferenced === 15
        && snapshot.releaseOutputPathCount === 32
  ...
  const guards = [
    ...
    },
    {
      ...
      satisfied: safariBlockers.some(item => /Current host cannot run Safari\/WebKit browser rows/.test(item)
        && /hostPlatform=[^,\)]+/.test(item)
        && /safaridriverFound=false/.test(item)
        && /currentHarnessSupportsSafari=true/.test(item)
