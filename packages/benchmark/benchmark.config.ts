...
export interface BenchmarkConfig {
  ...
  profiling: {
    enabled: boolean;
    methods: ('v8-prof' | 'inspector' | 'clinic')[];
    iterations: number;
  ...
  output: {
    resultsDir: string;
    formats: ('json' | 'csv' | 'markdown' | 'html' | 'console')[];
    saveRawSamples: boolean;
  };
...
export const comprehensiveConfig: BenchmarkConfig = {
  ...defaultConfig,
  warmupRuns: 5,
  measurementRuns: 20,
  testPatterns: defaultConfig.testPatterns.map(p => ({
...
export const ciConfig: BenchmarkConfig = {
  ...defaultConfig,
  warmupRuns: 2,
  measurementRuns: 5,
  timeout: 180,
...
export function mergeConfig(
  custom: Partial<BenchmarkConfig>,
  base: BenchmarkConfig = defaultConfig
): BenchmarkConfig {
  return {
    ...base,
    ...custom,
