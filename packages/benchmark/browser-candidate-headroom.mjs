...
async function startBenchmarkServer(options) {
  ...
  const server = createServer((request, response) => {
    ...
    if (url.pathname === '/corpus-cycle-stream') {
      streamCorpusCycle(response, options).catch(error => {
        if (!response.headersSent) {
          sendText(response, 500, 'text/plain; charset=utf-8', error?.stack ?? String(error));
        } else {
...
async function streamCorpusCycle(response, options) {
  const sourceBytes = statSync(options.corpusFile).size;
  if (sourceBytes <= 0) {
    throw new Error(`Corpus file is empty: ${options.corpusFile}`);
  }
  const targetBytes = Math.floor(options.sizeGiB * GIB);
  const actualBytes = Math.ceil(targetBytes / sourceBytes) * sourceBytes;
  response.writeHead(200, {
  ...
  let emittedBytes = 0;
  while (emittedBytes < actualBytes) {
    const stream = createReadStream(options.corpusFile);
...
function createReport(browserResult, options, hostProcessMemory) {
  ...
  return {
    ...
    sourceContract: {
      parserInput: 'Prepared full rows use StreamReaderSync over a synchronous Iterable<Uint8Array[]> generated by byteBatches(fixture); live fetch rows consume Response.body either through EventReader or manually grouped AsyncIterable<Uint8Array[]> batches.',
      batchBackpressure: 'byteBatches(fixture) yields one grouped batch per synchronous parser pull. fetchAsyncByteBatchFull groups Response.body with reader.read() only while filling the next yielded Uint8Array[] batch, so it is demand-driven and does not eagerly drain the stream.',
      fetchReadableStream: browserResult.fixture.source === 'corpus-file'
        ? 'fetchReadableStreamFull consumes fetch Response.body directly with EventReader while the local server repeats corpus bytes to the target size; fetchAsyncByteBatchFull manually groups Response.body reads into AsyncIterable<Uint8Array[]> batches under the same contract.'
        : 'fetchReadableStreamFull is available only for corpus-cycle fixtures.',
      corpusScope: browserResult.fixture.source === 'corpus-file'
        ? 'Most corpus rows still load fetch(...).arrayBuffer() as a seed and replay it; fetchReadableStreamFull and fetchAsyncByteBatchFull are live browser streaming source rows.'
        : 'The generated fixture rows are prepared before timing and replayed to the target size; it is not a browser network/file streaming proof.',
    },
    packageVersion,
    environment: {
...
function createFindings(variants, fixture, hostProcessMemory) {
  ...
  const findings = [
    {
      ...
      summary: fixture.source === 'corpus-file'
        ? 'Prepared rows consume corpus-backed browser Uint8Array batches through a synchronous Iterable<Uint8Array[]> and live fetch rows consume Response.body without loading a full XML string.'
        : 'Rows consume generated browser Uint8Array batches through a synchronous Iterable<Uint8Array[]> and do not load a full XML string.',
      evidence: [
        `fixtureSource=${fixture.source}`,
        `rowCycleSize=${fixture.rowCycleSize}`,
        `batchSize=${fixture.batchSize}`,
        ...fullRows.map(entry => `${entry.id}: maxJsHeap=${formatBytes(entry.memory.maxJsHeapUsedBytes)}`),
      ],
    },
    {
      id: 'browser-streaming-source-gap',
      summary: fixture.source === 'corpus-file'
        ? 'Most corpus-cycle rows still load a fixture seed with arrayBuffer() before replay; fetchReadableStreamFull and fetchAsyncByteBatchFull separately measure direct fetch Response.body consumption.'
        : 'Generated browser rows are prepared before timing and replayed by byteBatches(fixture); they are not true fetch/file streaming source rows.',
      evidence: [
  ...
  if (fixture.source === 'corpus-file') {
    findings.push({
      id: 'browser-fetch-readable-stream-source',
      summary: 'Fetch streaming rows consume the browser fetch body under the same full event-object checksum contract.',
      evidence: fullRows
        .filter(entry => entry.id === 'fetchReadableStreamFull' || entry.id === 'fetchAsyncByteBatchFull')
        .map(entry => `${entry.id}=${formatRate(entry.mibPerSec)}, events=${entry.eventCount}, checksum=${entry.checksum}`),
    });
    findings.push({
...
function renderMarkdown(report) {
  ...
  const lines = [
    ...
    `- Fetch ReadableStream: ${report.sourceContract.fetchReadableStream}`,
    `- Corpus/generated scope: ${report.sourceContract.corpusScope}`,
    '',
    '## Woodstox Target',
    '',
...
function createRunnerScript(options) {
  ...
  return `
import { EventReader, StreamEventType, StreamReaderSync, XmlEventType, createEventReaderFromAsyncByteBatches } from '/stax/index.js';
...
async function runBenchmark() {
  ...
  const variants = [];
  for (const variant of filterVariants(createVariants(fixture), config.cases)) {
    variants.push(await measureVariant(variant, fixture, config));
  }
  return {
...
function createVariants(fixture) {
  ...
  if (fixture.source === 'corpus-file') {
    variants.push(
      {
        id: 'fetchReadableStreamFull',
        family: 'readable-stream-live-source',
        implementation: 'EventReader over fetch Response.body ReadableStream corpus-cycle source',
        contractScope: 'full-event-object-materialization-live-readable-stream-source',
        fullStringParity: true,
        ...
        run: () => consumeFetchEventReaderFull(fixture),
      },
      {
        id: 'fetchAsyncByteBatchFull',
        family: 'async-byte-batch-live-source',
        implementation: 'createEventReaderFromAsyncByteBatches over fetch Response.body corpus-cycle source',
        contractScope: 'full-event-object-materialization-live-async-byte-batch-source',
        fullStringParity: true,
        ...
        respectsBackpressure: true,
        fullArrayBufferParserInput: false,
        run: () => consumeFetchAsyncByteBatchFull(fixture),
      },
    );
  }
...
async function measureVariant(variant, fixture, options) {
  ...
  return {
    ...
    sourceMode: variant.sourceMode ?? null,
    demandDrivenSource: variant.demandDrivenSource ?? null,
    directReadableStream: variant.directReadableStream ?? null,
    respectsBackpressure: variant.respectsBackpressure ?? null,
    fullArrayBufferParserInput: variant.fullArrayBufferParserInput ?? null,
    avgMs,
...
async function consumeFetchEventReaderFull(fixture) {
  const response = await fetch('/corpus-cycle-stream');
  if (!response.ok || !response.body) {
    throw new Error('Failed to fetch corpus-cycle stream: HTTP ' + response.status);
  }
  const materializationCounters = createMaterializationCounters();
  const objectSink = new Array(1024);
  let objectSinkIndex = 0;
  ...
  for await (const event of new EventReader(response.body, { batchSize: fixture.batchSize })) {
  ...
  globalThis.__staxBrowserCandidateFetchEventObjectSink = objectSink[(objectSinkIndex - 1) & (objectSink.length - 1)];
  return { eventCount, checksum, materializationCounters };
}

async function consumeFetchAsyncByteBatchFull(fixture) {
  const response = await fetch('/corpus-cycle-stream');
  if (!response.ok || !response.body) {
    throw new Error('Failed to fetch corpus-cycle stream: HTTP ' + response.status);
  }
  ...
  for await (const event of createEventReaderFromAsyncByteBatches(fetchBodyByteBatches(response.body, fixture.batchSize))) {
    materializationCounters.eventObjects++;
    objectSink[objectSinkIndex & (objectSink.length - 1)] = event;
    objectSinkIndex++;
  ...
  globalThis.__staxBrowserCandidateFetchAsyncBatchObjectSink = objectSink[(objectSinkIndex - 1) & (objectSink.length - 1)];
  return { eventCount, checksum, materializationCounters };
}

async function* fetchBodyByteBatches(body, batchSize) {
