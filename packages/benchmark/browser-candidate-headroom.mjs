  ...
  return {
    ...
    sourceContract: {
      ...
      corpusScope: browserResult.fixture.source === 'corpus-file'
        ? 'The corpus fixture is loaded once with fetch(...).arrayBuffer() as a seed and replayed to the target size; it is not a browser network/file streaming proof.'
...
function createFindings(variants, fixture, hostProcessMemory) {
  ...
  const findings = [
    ...
    },
    {
      ...
      summary: 'The browser matrix avoids ReadableStream async iteration overhead, but corpus-cycle currently loads a fixture seed with arrayBuffer() before replay; it is not yet a true fetch/file streaming source benchmark.',
      evidence: [
        fixture.source === 'corpus-file'
          ? 'createFixtureRows(corpus-cycle) returns [new Uint8Array(await response.arrayBuffer())].'
