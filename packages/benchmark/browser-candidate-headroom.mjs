...
function createReport(browserResult, options, hostProcessMemory) {
  ...
  return {
    ...
    sourceContract: {
      parserInput: 'StreamReaderSync consumes a synchronous Iterable<Uint8Array[]> generated by byteBatches(fixture).',
      ...
      fetchReadableStream: browserResult.fixture.source === 'corpus-file'
        ? 'fetchReadableStreamFull consumes fetch Response.body directly with EventReader while the local server repeats corpus bytes to the target size.'
        : 'fetchReadableStreamFull is available only for corpus-cycle fixtures.',
      corpusScope: browserResult.fixture.source === 'corpus-file'
        ? 'Most corpus rows still load fetch(...).arrayBuffer() as a seed and replay it; fetchReadableStreamFull is the live browser ReadableStream source row.'
...
function createFindings(variants, fixture, hostProcessMemory) {
  ...
  const findings = [
    {
      ...
      summary: fixture.source === 'corpus-file'
        ? 'Rows consume corpus-backed browser Uint8Array batches through a synchronous Iterable<Uint8Array[]> and do not load a full XML string.'
    ...
    },
    {
      ...
      summary: fixture.source === 'corpus-file'
        ? 'Most corpus-cycle rows still load a fixture seed with arrayBuffer() before replay; fetchReadableStreamFull separately measures direct fetch Response.body consumption.'
  ...
  if (fixture.source === 'corpus-file') {
    findings.push({
      ...
      summary: 'fetchReadableStreamFull consumes the browser fetch body as a ReadableStream under the same full event-object checksum contract.',
      evidence: fullRows
        .filter(entry => entry.id === 'fetchReadableStreamFull')
        .map(entry => `fetchReadableStreamFull=${formatRate(entry.mibPerSec)}, events=${entry.eventCount}, checksum=${entry.checksum}`),
    });
    findings.push({
...
function createRunnerScript(options) {
  ...
  return `
import { EventReader, StreamEventType, StreamReaderSync, XmlEventType } from '/stax/index.js';
