...
async function loadParserVariants(): Promise<ParserVariant[]> {
...
// Load test files
function loadTestFiles(): Map<string, string> {
...
function countXmlEvents(xml: string, parser: typeof StaxXmlParserSync): number {
  let eventCount = 0;
  const p = new parser();
...
async function runMitataBenchmarks(variants: ParserVariant[], testFiles: Map<string, string>) {
...
function generateComparison(allResults: BenchmarkResult[]): string {
...
async function main() {
  ...
  if (testFiles.size === 0) {
    console.error('No test files found. Please run generate-test-xmls.ts first.');
    process.exit(1);
  }
...
export { main as runInliningBenchmarks, BenchmarkResult, BenchmarkConfig };