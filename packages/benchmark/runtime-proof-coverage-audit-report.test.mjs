...
test('runtime proof coverage audit keeps open proof obligations explicit', () => {
  ...
  assert.equal(report.summary.scannedArtifactCount, 65);
  assert.equal(report.summary.measuredRowCount, 378);
  assert.equal(report.summary.largeJsFullRowCount, 169);
  ...
  assert.equal(report.coverage.browser.firefoxBenchmarkRows.length, 36);
  assert.equal(report.coverage.browser.safariBenchmarkRows.length, 0);
  assert.equal(report.coverage.browser.nonV8BenchmarkRows.length, 36);
  ...
  assert.match(markdown, /36 Firefox\/SpiderMonkey browser benchmark rows found/);
  ...
  assert.match(markdown, /Non-V8 browser benchmark rows: 36/);
