...
test('runtime proof coverage audit keeps open proof obligations explicit', () => {
  ...
  assert.equal(report.coverage.browser.firefoxBenchmarkRows.length, 51);
  assert.equal(report.coverage.browser.safariBenchmarkRows.length, 0);
  assert.equal(report.coverage.browser.nonV8BenchmarkRows.length, 51);
  ...
  assert.match(markdown, /51 Firefox\/SpiderMonkey browser benchmark rows found/);
  ...
  assert.match(markdown, /Non-V8 browser benchmark rows: 51/);
