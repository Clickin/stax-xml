...
test('runtime proof coverage audit keeps open proof obligations explicit', () => {
  ...
  assert.equal(report.summary.scannedArtifactCount, 123);
  ...
  assert.equal(report.summary.benchmarkArtifactCount, 86);
  ...
  assert.equal(report.summary.negativeArtifactCount, 7);
