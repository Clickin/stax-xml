...
const source = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><?before value?><!DOCTYPE root><!--note--><root xmlns:p="urn:p"><p:empty xmlns:q="urn:q" q:attr="value"/><text><![CDATA[value]]></text></root><?after?>';
...
describe('event writer pipeline', () => {
  ...
  it('writes the same event stream to sync sinks and async writers', async () => {
    ...
    const sync = new WriterSyncSink({ write: chunk => syncChunks.push(chunk) });
    for (const event of events) sync.writeEvent(event);
    expect(syncChunks.join('')).toBe(source);

    const asyncChunks: string[] = [];
    const async = new Writer({ encoding: 'UTF-8', write: chunk => asyncChunks.push(chunk) });
  ...
  it('preserves an XML declaration split across async input chunks', async () => {
    ...
    const chunks: string[] = [];
    const writer = new Writer({ encoding: 'UTF-8', write: chunk => chunks.push(chunk) });
    for await (const event of new EventReader(input())) await writer.writeEvent(event);
  ...
  it('supports filtering and replacing events in a pipeline', () => {
    const writer = new WriterSync();
    for (const event of new EventReaderSync('<root><!--discard--><item>old</item></root>')) {
      if (event.type === XmlEventType.COMMENT) continue;
      writer.writeEvent(event.type === XmlEventType.CHARACTERS ? { ...event, value: 'new' } : event);
    }
  ...
  it('requires the suppressed end event for a self-closing element to match', () => {
    ...
    if (end?.type !== XmlEventType.END_ELEMENT) throw new Error('expected END_ELEMENT');
    const writer = new WriterSync();

    for (const event of events) {
      if (event !== end) writer.writeEvent(event);
      else expect(() => writer.writeEvent({ ...end, name: 'other', localName: 'other' })).toThrow(/self-closing|matching/i);
    }
  });
