...
describe("v1 current-token reader contract", () => {
  it("uses the configured fatal TextDecoder encoding for byte readers", async () => {
    ...
    expect(collectSync(littleEndian, { encoding: "utf-16le" })).toEqual(
      expected,
    );
    expect(
      Array.from(
        new EventReaderSync(littleEndian, { encoding: "utf-16le" }),
      ).map(eventToken),
    ).toEqual(Array.from(new EventReaderSync(xml)).map(eventToken));
    await expect(
      collectAsync(byteChunks(bigEndian, 3), { encoding: "utf-16be" }),
    ).resolves.toEqual(expected);

    const asyncEvents: unknown[][] = [];
    for await (const event of new EventReader(byteChunks(bigEndian, 3), {
      encoding: "utf-16be",
    })) {
      asyncEvents.push(eventToken(event));
    }
    expect(asyncEvents).toEqual(
      Array.from(new EventReaderSync(xml)).map(eventToken),
    );

    expect(() =>
      collectSync(littleEndian.slice(0, -1), { encoding: "utf-16le" }),
    ).toThrow();
    expect(
      () => new StreamReaderSync(littleEndian, { encoding: "not-an-encoding" }),
    ).toThrow();
    expect(
      () =>
        new StreamReader(byteChunks(bigEndian, 3), {
  ...
  it("can disable namespace processing for raw qualified-name workloads", async () => {
    ...
    expect([...new EventReaderSync(xml, options)].map(eventToken)).toEqual(
      expected,
    );

    const asyncEvents: unknown[][] = [];
    for await (const event of new EventReader(asyncChunks(xml), options))
      asyncEvents.push(eventToken(event));
    expect(asyncEvents).toEqual(expected);

    const reader = new StreamReaderSync(xml, options);
    while (
      reader.next() !== XmlEventType.START_ELEMENT ||
      reader.name() !== "p:item"
    ) {
      /* advance */
    }
    ...
    expect(() => collectSync("<p:raw/>")).toThrow(
      /undeclared namespace prefix/i,
    );
  });

  it("decodes predefined and numeric character references", () => {
    expect(
      collectSync('<r a="&quot;&#65;&#x42;">&lt;&amp;&gt;&#x1F600;</r>'),
    ).toContainEqual([XmlEventType.CHARACTERS, "<&>😀"]);
    const reader = new StreamReaderSync('<r a="&quot;&#65;&#x42;"/>');
    while (reader.next() !== XmlEventType.START_ELEMENT) {
      /* advance */
    }
  ...
  it("can preserve raw entity spelling while still validating references", async () => {
    const xml =
      '<r a="&#x398;&product;">&#920;&product;&amp;product;<![CDATA[&product;]]></r>';
    const options = {
    ...
    expect(collectSync(xml, options)).toEqual(expected);
    await expect(collectAsync(asyncChunks(xml), options)).resolves.toEqual(
      expected,
    );
    expect(
      collectSync('<r xmlns:p="urn:&product;"><p:x/></r>', options),
    ).toContainEqual([
      XmlEventType.START_ELEMENT,
      "p:x",
      "x",
    ...
    expect(
      collectSync('<r xmlns:p="urn:&product;"/>', {
        ...options,
    ...
    expect(() =>
      collectSync("<r>&unknown;</r>", { autoDecodeEntities: false }),
    ).toThrow(/unknown|entity/i);
    expect(() =>
      collectSync("<r>&#0;</r>", { autoDecodeEntities: false }),
    ).toThrow(/unknown|invalid|entity/i);
  });

  it("uses trusted custom entities without recursive expansion or DTD processing", async () => {
    const xml =
      '<r a="&product;">&product;&amp;product;<![CDATA[&product;]]></r>';
    const options = {
      addEntities: [{ entity: "&product;", value: "StAX & XML" }],
    };
    const expected = [
  ...
  it("defaults to fragment mode and enforces document mode when requested", () => {
    ...
    expect(() =>
      collectSync("<a/><b/>", { documentMode: "document" }),
    ).toThrow();
    expect(() =>
      collectSync("<a/>tail", { documentMode: "document" }),
    ).toThrow();
  });

  it("produces the same tokens for strings, bytes, and every-byte chunking", () => {
    const xml =
      '<!DOCTYPE r [<!ELEMENT r ANY>]><!--note--><?work now?><r xmlns:p="u"><p:x a="&amp;😀"> one <![CDATA[<two>]]></p:x></r>';
  ...
  it("rejects duplicate expanded attribute names on narrow and wide tags", () => {
    expect(() =>
      collectSync('<r xmlns:a="u" xmlns:b="u" a:x="1" b:x="2"/>'),
    ).toThrow(/duplicate/i);
    const attributes = Array.from(
      { length: 17 },
      (_, index) => `a:x${index}="${index}"`,
    );
    attributes.push('b:x0="duplicate"');
    expect(() =>
      collectSync(`<r xmlns:a="u" xmlns:b="u" ${attributes.join(" ")}/>`),
    ).toThrow(/duplicate/i);
  });
  ...
  it("scans string input without requiring TextEncoder", () => {
    ...
    try {
      expect(collectSync("<root>한글😀</root>")).toContainEqual([
        XmlEventType.CHARACTERS,
        "한글😀",
      ]);
    } finally {
  ...
  it("exposes comment, processing-instruction, and DTD tokens without DOM buffering", () => {
    const tokens = collectSync(
      "<!DOCTYPE root [<!ELEMENT root ANY>]><!--note--><?work now?><root/>",
    );
  ...
  it("restores namespace bindings after a nested shadowing scope", () => {
    ...
    while (reader.next() !== null) {
      ...
      starts.set(name, {
        ...
        qualifiedAttributeURI:
          name === "p:c" ? reader.attributeNamespaceURI(0) : undefined,
        unqualifiedAttributeURI:
          name === "p:c" ? reader.attributeNamespaceURI(1) : undefined,
        qualifiedAttributeValue:
          name === "p:c" ? reader.attributeValue("urn:p1", "x") : undefined,
        unqualifiedAttributeValue:
          name === "p:c" ? reader.attributeValue("", "x") : undefined,
      });
    }

    expect(starts.get("b")).toMatchObject({
      uri: "urn:inner",
    ...
    expect(starts.get("p:d")).toMatchObject({
      uri: "urn:p0",
      defaultURI: "urn:outer",
      pURI: "urn:p0",
    });
  });
    ...
    while (reader.next() !== null) {
      if (
        ...
        itemEvents.push([
          reader.eventType(),
          reader.namespaceURI(),
          reader.namespaceURIForPrefix("p"),
        ]);
      }
    }
  ...
  it("exhausts 1,000 nested unique namespace-prefix scopes correctly", () => {
    ...
    const reader = new StreamReaderSync(
      `<root>${starts.join("")}${ends.join("")}</root>`,
    );
    let startIndex = 0;
    ...
    while (reader.next() !== null) {
      ...
      if (reader.eventType() === XmlEventType.START_ELEMENT) {
        ...
        expect(reader.namespaceURIForPrefix(`p${startIndex}`)).toBe(
          `urn:${startIndex}`,
        );
        startIndex++;
        ...
        expect(reader.namespaceURIForPrefix(`p${endIndex}`)).toBe(
          `urn:${endIndex}`,
        );
        endIndex--;
      }
    ...
    expect({ startIndex, endIndex }).toEqual({
      startIndex: depth,
      endIndex: -1,
    });
  });
  ...
  it.each([64, 256])(
    "accepts %i unique attributes including a final hash collision",
    (count) => {
  ...
  it.each([64, 256])(
    "rejects a duplicate attribute in the final slot of %i attributes",
    (count) => {
      const names = Array.from({ length: count }, (_, index) => `a${index}`);
      ...
      expect(() => collectSync(`<root ${attributes}/>`)).toThrow(
  ...
  it.each([
    ...
    [
      "reserved XML URI on another prefix",
  ...
  it("keeps previously returned strings valid after advancing", () => {
    const reader = new StreamReaderSync(
      '<root><first attr="value">text</first></root>',
    );
    while (
      reader.next() !== XmlEventType.START_ELEMENT ||
      reader.name() !== "first"
...
describe("v1 sync/async parity and async lifecycle", () => {
  it("rejects invalid sync iterable chunks before batching and closes the source", () => {
    ...
    const reader = new StreamReaderSync(
      invalidChunks() as Iterable<Uint8Array>,
    );
    expect(() => {
      while (reader.next() !== null) {
        /* exhaust */
  ...
  it("rejects concurrent next calls", async () => {
    ...
    release();
    await expect(first).resolves.toBe(XmlEventType.START_DOCUMENT);
  });
  ...
  it("closes an async source exactly once after a parse error", async () => {
    ...
    async function* source(): AsyncGenerator<Uint8Array> {
      try {
        yield encoder.encode("<root></wrong>");
      } finally {
        returned++;
      }
    }
...
describe("v1 event reader contract", () => {
  it("accepts strings and byte chunks with identical sync results", () => {
    ...
    expect(Array.from(new EventReaderSync(chunks(xml)))).toEqual(
      Array.from(new EventReaderSync(xml)),
    );
  });
  ...
  it("closes a sync source exactly once on return and parse error", () => {
    ...
    function* earlySource(): Generator<Uint8Array> {
      try {
        yield encoder.encode("<root><child/></root>");
      } finally {
        earlyReturns++;
      }
    }
  ...
  it("materializes stable sync events", () => {
    const reader = new EventReaderSync(
      '<root><item id="1">one</item><item id="2">two</item></root>',
    );
    const events = Array.from(reader);
    const firstItem = events.find(
      (event) =>
        event.type === XmlEventType.START_ELEMENT && event.name === "item",
    );
    const firstText = events.find(
      (event) => event.type === XmlEventType.CHARACTERS,
    );
    ...
    if (firstItem?.type !== XmlEventType.START_ELEMENT)
      throw new Error("expected start element");
    expect(firstItem.attributes.get("id")).toMatchObject({
      name: "id",
      localName: "id",
      value: "1",
    });
  ...
  it("indexes materialized attributes safely and serializes them as JSON objects", () => {
    const event = Array.from(
      new EventReaderSync(
        '<root first="1" __proto__="safe" constructor="safe-too"/>',
      ),
    ).find((candidate) => candidate.type === XmlEventType.START_ELEMENT);
    if (event?.type !== XmlEventType.START_ELEMENT)
      throw new Error("expected start element");

    expect(Array.from(event.attributes.keys())).toEqual([
      "first",
      "__proto__",
      "constructor",
    ]);
    ...
    const json = JSON.parse(JSON.stringify(event)) as {
      attributes: Record<string, EventAttribute>;
    };
  ...
  it("materializes comment, processing-instruction, and DTD payloads", () => {
    const events = Array.from(
      new EventReaderSync("<!DOCTYPE root><!--note--><?work now?><root/>"),
    );
  ...
  it("keeps sync and async event shapes equivalent across chunk boundaries", async () => {
    ...
    const asyncEvents = [];
    for await (const event of new EventReader(asyncChunks(xml)))
  ...
  it("matches StreamReader tokens without reading beyond the same async source", async () => {
    const xml =
      '<!DOCTYPE r><!--note--><?work now?><r xmlns:p="u"><p:x id="&amp;"/> text </r>';
    const streamTokens = await collectAsync(asyncChunks(xml));
    const eventTokens: unknown[][] = [];

    for await (const event of new EventReader(asyncChunks(xml)))
      eventTokens.push(eventToken(event));
  ...
  it("cancels the EventReader source when iteration returns early", async () => {
    ...
    await expect(reader.return()).resolves.toEqual({
      value: undefined,
      done: true,
    });
    expect(returned).toBe(true);
  ...
  it("exposes idempotent close methods on both event readers", async () => {
    ...
    await expect(asyncReader.next()).resolves.toEqual({
      value: undefined,
      done: true,
    });
  });
  ...
  it("does not read ahead while buffered EventReader events remain", async () => {
    ...
    await expect(pending).resolves.toMatchObject({
      ...
      value: { type: XmlEventType.END_ELEMENT, name: "root" },
    });
    expect(pulls).toBe(2);
  });
  ...
  it("keeps prior async IteratorResult events and attributes stable after advancing", async () => {
    ...
    while (true) {
      ...
      if (
        result.value.type === XmlEventType.START_ELEMENT &&
        result.value.name === "item"
      ) {
        saved = result;
        break;
      }
