export type Uint8ArrayCurrentCursorMaterializationPolicy =
  | 'none'
  | 'current-event'
  | 'eager-touch';

export interface Uint8ArrayCurrentCursorOptions {
  materialization?: Uint8ArrayCurrentCursorMaterializationPolicy;
...
export class Uint8ArrayCurrentCursor {
  ...
  private readonly materialization: Uint8ArrayCurrentCursorMaterializationPolicy;
  private readonly implicitAttributeValue: 'true' | 'name';

  constructor(
    source: Iterable<readonly Uint8Array[]>,
    options: Uint8ArrayCurrentCursorOptions = {},
  ) {
  ...
  name(): string | undefined {
    ...
    if (this.materialization === 'none') {
      return decodeUtf8(this.currentBuffer, this.currentNameStart, this.currentNameEnd);
    }
    ...
    this.memoName = decodeUtf8(this.currentBuffer, this.currentNameStart, this.currentNameEnd);
  ...
  text(): string | undefined {
    ...
    if (this.materialization === 'none') {
      return decodeUtf8(this.currentBuffer, this.currentTextStart, this.currentTextEnd);
    }
    if (this.materialization === 'eager-touch') {
      this.hydrateCurrentEvent();
    ...
    this.memoText = decodeUtf8(this.currentBuffer, this.currentTextStart, this.currentTextEnd);
  ...
  getAttributeName(index: number): string | undefined {
    ...
    if (this.materialization === 'none') {
      return decodeUtf8(this.currentBuffer, this.attrNameStarts[index]!, this.attrNameEnds[index]!);
    }
    if (this.materialization === 'eager-touch') {
      this.hydrateCurrentEvent();
    ...
    this.memoAttrName = decodeUtf8(this.currentBuffer, this.attrNameStarts[index]!, this.attrNameEnds[index]!);
  ...
  private parseEndTag(position: number, isFinal: boolean): number {
    ...
    if (this.elementStack.length === 0) {
      throw new Error(`Mismatched closing tag: </${decodeUtf8(this.currentBuffer, nameStart, nameEnd)}>. No open elements.`);
    }

    const found = decodeUtf8(this.currentBuffer, nameStart, nameEnd);
  ...
  private parseStartTag(position: number, isFinal: boolean): number {
    ...
    const name = decodeUtf8(this.currentBuffer, nameStart, nameEnd);
    if (selfClosing) {
  ...
  private sliceAttributeValue(index: number): string {
    if (this.attrImplicit[index] === 1) {
      return this.implicitAttributeValue === 'name'
        ? decodeUtf8(this.currentBuffer, this.attrNameStarts[index]!, this.attrNameEnds[index]!)
        : 'true';
    }
    return decodeUtf8(this.currentBuffer, this.attrValueStarts[index]!, this.attrValueEnds[index]!);
  }
  ...
  private hydrateCurrentEvent(): void {
    ...
    this.eagerName = this.currentNameStart < 0
      ? undefined
      : decodeUtf8(this.currentBuffer, this.currentNameStart, this.currentNameEnd);
    this.eagerText = this.currentTextStart < 0
      ? undefined
      : decodeUtf8(this.currentBuffer, this.currentTextStart, this.currentTextEnd);
    ...
    for (let index = 0; index < this.attrCount; index++) {
      attrNames[index] = decodeUtf8(this.currentBuffer, this.attrNameStarts[index]!, this.attrNameEnds[index]!);
      attrValues[index] = this.attrImplicit[index] === 1
        ? (this.implicitAttributeValue === 'name' ? attrNames[index]! : 'true')
        : decodeUtf8(this.currentBuffer, this.attrValueStarts[index]!, this.attrValueEnds[index]!);
    }
  ...
  private prepareBuffer(batch: readonly Uint8Array[]): Uint8Array {
    ...
    const buffers = batch.map(asUint8ArrayView);
    if (!hasTail) {
      return concatUint8Arrays(buffers, total);
    }

    const chunks: Uint8Array[] = [this.pendingTail];
...
function asUint8ArrayView(source: Uint8Array): Uint8Array {
  return source;
}

function concatUint8Arrays(chunks: readonly Uint8Array[], total: number): Uint8Array {
...
function isXmlnsAttributeName(buffer: Uint8Array, start: number, end: number): boolean {
...
function indexOfAscii(buffer: Uint8Array, value: string, offset: number): number {
  const first = value.charCodeAt(0);
  const max = buffer.byteLength - value.length;
  for (let index = offset; index <= max; index++) {
    if (buffer[index] !== first) continue;
    let matched = true;
    for (let valueIndex = 1; valueIndex < value.length; valueIndex++) {
      if (buffer[index + valueIndex] !== value.charCodeAt(valueIndex)) {
        matched = false;
        break;
      }
    }
...
function findGt(buffer: Uint8Array, start: number): number {
  return buffer.indexOf(62, start);
}

function findTagEnd(buffer: Uint8Array, start: number): number {
  let inQuote = false;
  let quote = 0;
...
function findDoctypeEnd(buffer: Uint8Array, start: number): number {
  let depth = 0;
  let quote = 0;
  for (let index = start; index < buffer.byteLength; index++) {
    const byte = buffer[index]!;
