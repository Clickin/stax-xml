...
export class XPathCompiler {
  ...
  private static compileInternal(xpath: string): CompiledXPath {
    ...
    if (
      trimmed !== "." &&
      (!path || path.split("/").some((part) => part.length === 0))
    ) {
    ...
    for (let index = 0; index < parts.length; index++) {
      ...
      if (
        (segment.isAttribute || segment.isTextNode) &&
        index !== parts.length - 1
      ) {
        throw new Error(
          `XPath attributes and text() must be terminal: ${xpath}`,
        );
      }
    ...
    if (
      (isAbsolute || isDescendant) &&
      segments.length === 1 &&
      (terminal?.isAttribute || terminal?.isTextNode)
    ) {
  ...
  private static compileSegment(segment: string): XPathSegment {
    ...
    if (predicateMatch[2]) {
      ...
      if (!Number.isSafeInteger(position) || position < 1) {
        throw new Error(
          `XPath position must be a positive safe integer: ${segment}`,
        );
      }
