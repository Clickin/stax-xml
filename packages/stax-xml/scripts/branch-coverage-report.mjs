...
const rows = Object.entries(coverage)
  ...
  .sort(
    (left, right) =>
      left.percent - right.percent || left.file.localeCompare(right.file),
  );

for (const row of rows)
  console.log(
    `${row.percent.toFixed(2)}% ${row.covered}/${row.total} ${row.file}`,
  );
...
console.log(
  `${status} ${percent.toFixed(2)}% ${covered}/${total} overall (minimum ${minimum}%).`,
);

if (fail && percent < minimum) {
  console.error(
    `branch coverage gate failed: ${percent.toFixed(2)}% is below ${minimum}%.`,
  );
  process.exit(1);
}
