...
## v1.1: Read → Modify → Write

`writeEvent()`가 `EventReader`와 `EventReaderSync`를 모든 writer에 직접
연결합니다. 필요한 event만 교체하는 동안 XML declaration metadata, namespace
declaration, attribute, self-closing 표기를 유지할 수 있습니다. 할당에 민감한
변환에서는 `StreamReader`와 writer의 개별 stream method를 조합하세요.

[XML 변환 파이프라인 만들기 →](/stax-xml/ko/guide/event-pipelines/)
