...
head:
  ...
  - tag: meta
    attrs:
      name: twitter:image
      content: https://clickin.github.io/stax-xml/og/api-guides/staxxml-writer.png
slug: v0.7.0/api-guides/staxxml-writer
---
...
For large file or large document output where maximum write throughput matters, prefer [`StaxXmlWriterSyncSink`](/stax-xml/v0.7.0/api-guides/staxxml-writer-sync/#sink-based-incremental-writing). The 1GiB writer benchmark shows the sync sink path has the best write throughput while peak RSS stays in the same range as async writing.
