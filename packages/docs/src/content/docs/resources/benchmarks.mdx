...
- Because the converter targets XPath 1.0 selectors plus field transforms, most selector-style `if` branches can move into declarative predicates, positions, descendant paths, and attribute tests.
- Keep `EventReader` or `StreamReader` when the logic depends on stream order, early exit, side effects, or mutable cross-record state that XPath selection cannot express.
...
The `parser-*` sections compare public stax-xml surfaces that all produce the same final JavaScript object shape for each fixture. Read them together, in fixture-size order, before comparing the separate iterable file-traversal and runtime matrices.
...
- **EventReaderSync**: string-event convenience surface with manual object materialization
- **StreamReaderSync**: byte-oriented pull surface with manual object materialization
- **ProjectionReader**: projection-oriented surface for unknown-schema extraction that still returns the same final objects here
- **Converter**: declarative schema surface; use this first when the target object shape is already known
- **Very large files (1GB+)**: `EventReader` streams avoid retaining one full XML string; the parser fixture series above remains an in-memory sync comparison


### EventReader Stream Traversal (1MiB to 1GiB default)

stax-xml native traversal over bounded file chunks:
...
- The memory column in the rendered table is peak JS heap for the traversal run. Peak RSS is tracked in the raw JSON and must be measured in an isolated process to stay meaningful.
